...
3. Configure CmlHiveAssist with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for CmlHiveAssist + `imsg` (Messages DB access).
...
2. Create a macOS user (example: `cml-hive-assisthome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. CmlHiveAssist only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. CmlHiveAssist can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, CmlHiveAssist attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - cml-hive-assist gateway           │          SCP (attachments)        │ - Messages signed in     │
...
Example wrapper (`~/.cml-hive-assist/scripts/imsg-ssh`):
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, CmlHiveAssist treats that thread as a “group” for:
