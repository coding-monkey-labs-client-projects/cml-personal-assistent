...
```bash
cml-hive-assist plugins install @cml-hive-assist/msteams
```

Local checkout (when running from a git repo):

```bash
cml-hive-assist plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
CmlHiveAssist will offer the local install path automatically.
...
3. Configure CmlHiveAssist with those credentials.
...
- Talk to CmlHiveAssist via Teams DMs, group chats, or channels.
...
- On startup, CmlHiveAssist resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.cml-hive-assist/cml-hive-assist.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring CmlHiveAssist, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `cml-hive-assist-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `cml-hive-assist plugins install @cml-hive-assist/msteams`
   - From a local checkout: `cml-hive-assist plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "CmlHiveAssist in Teams", "full": "CmlHiveAssist in Teams" },
...
CmlHiveAssist handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, CmlHiveAssist only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/CmlHiveAssistShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

CmlHiveAssist sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `cml-hive-assist message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.cml-hive-assist/msteams-polls.json`.
...
```bash
cml-hive-assist message send --channel msteams \
...
cml-hive-assist message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
cml-hive-assist message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
cml-hive-assist message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
cml-hive-assist message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
