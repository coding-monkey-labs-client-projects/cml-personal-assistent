...
CmlHiveAssist can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, CmlHiveAssist retries the same message as plain text.

## Commands (native + custom)

CmlHiveAssist registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; CmlHiveAssist does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `cml-hive-assist logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). CmlHiveAssist can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. CmlHiveAssist:
...
Private chats can include `message_thread_id` in some edge cases. CmlHiveAssist keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `cml-hive-assist logs --follow` and look for `from.id`.
...
CmlHiveAssist defaults to audio files for backward compatibility.
...
CmlHiveAssist supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, CmlHiveAssist handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, CmlHiveAssist caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.cml-hive-assist/telegram/sticker-cache.json`
...
CmlHiveAssist uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, CmlHiveAssist:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by CmlHiveAssist)
...
- Example: `cml-hive-assist message send --channel telegram --target 123456789 --message "hi"`.
...
- `cml-hive-assist channels status` shows a warning when config expects unmentioned group messages.
- `cml-hive-assist channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `cml-hive-assist logs --follow` (look for "skipping group message")
...
- Upgrade to a CmlHiveAssist build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
