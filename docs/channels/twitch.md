...
read_when:
  - Setting up Twitch chat integration for CmlHiveAssist
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. CmlHiveAssist connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
cml-hive-assist plugins install @cml-hive-assist/twitch
```

Local checkout (when running from a git repo):

```bash
cml-hive-assist plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `CML_HIVE_ASSIST_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "cml-hive-assist", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CML_HIVE_ASSIST_TWITCH_ACCESS_TOKEN env var)
...
```bash
CML_HIVE_ASSIST_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
