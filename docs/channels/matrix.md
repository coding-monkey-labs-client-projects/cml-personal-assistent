...
Matrix is an open, decentralized messaging protocol. CmlHiveAssist connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `cml-hive-assist plugins install @cml-hive-assist/matrix`
   - From a local checkout: `cml-hive-assist plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: CmlHiveAssist calls the same
     login endpoint, stores the access token in `~/.cml-hive-assist/credentials/matrix/credentials.json`,
...
- On first connection, CmlHiveAssist requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  CmlHiveAssist logs a warning.
...
`~/.cml-hive-assist/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, CmlHiveAssist resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
