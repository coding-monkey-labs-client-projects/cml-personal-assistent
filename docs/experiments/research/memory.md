...
read_when:
  - Designing workspace memory (~/.cml-hive-assist/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep CmlHiveAssist integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.cml-hive-assist/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.cml-hive-assist/workspace` as canonical human-readable memory.
...
```
~/.cml-hive-assist/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in CmlHiveAssist**, but keep a separable core library.

### Why integrate into CmlHiveAssist?

- CmlHiveAssist already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `cml-hive-assist memory recall "…" --k 25 --since 30d`
  - `cml-hive-assist memory reflect --since 7d`
...
Pragmatic take for `~/.cml-hive-assist/workspace`:
