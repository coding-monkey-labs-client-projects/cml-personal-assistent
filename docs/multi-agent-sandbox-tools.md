...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `cml-hive-assist sandbox explain`.
...
Legacy `agent.*` configs are migrated by `cml-hive-assist doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=cml-hive-assist-sbx-"
   ```
