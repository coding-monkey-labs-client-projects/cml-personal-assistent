...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.cml-hive-assist/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.cml-hive-assist/node.json` (node id + pairing token).
...
Path: `~/.cml-hive-assist/exec-approvals.json`
...
- Unix socket at `~/.cml-hive-assist/exec-approvals.sock` (0600).
