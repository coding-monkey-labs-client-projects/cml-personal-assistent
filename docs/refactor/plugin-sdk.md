...
- Publish as `cml-hive-assist/plugin-sdk` (or export from core under `cml-hive-assist/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `CmlHiveAssistPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: CmlHiveAssistConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: CmlHiveAssistConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: CmlHiveAssistConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: CmlHiveAssistConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: CmlHiveAssistConfig): string;
  };
};
```
...
- Introduce `cml-hive-assist/plugin-sdk`.
- Add `api.runtime` to `CmlHiveAssistPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `cml-hive-assistRuntime: ">=2026.2.0"`).
