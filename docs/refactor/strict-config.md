...
- Plugin manifests (`cml-hive-assist.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`cml-hive-assist.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `cml-hive-assist doctor --fix`.
...
- `cml-hive-assist help`
- `cml-hive-assist status`
- `cml-hive-assist gateway status`

Everything else must hard-fail with: “Config invalid. Run `cml-hive-assist doctor --fix`.”
