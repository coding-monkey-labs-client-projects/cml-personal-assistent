...
read_when:
  - You want to understand `cml-hive-assist.ai/install.sh`
...
CmlHiveAssist ships two installer scripts (served from `cml-hive-assist.ai`):

- `https://cml-hive-assist.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://cml-hive-assist.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
- `https://cml-hive-assist.ai/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://cml-hive-assist.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://cml-hive-assist.ai/install.ps1))) -?
```

If the installer completes but `cml-hive-assist` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g cml-hive-assist@latest`
...
- If upgrading an existing install: runs `cml-hive-assist doctor --non-interactive` (best effort).
- For git installs: runs `cml-hive-assist doctor --non-interactive` after install/update (best effort).
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://cml-hive-assist.ai/install.sh | bash
```

### Discoverability / “git install” prompt

If you run the installer while **already inside a CmlHiveAssist source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:
...
This script installs `cml-hive-assist` into a prefix (default: `~/.cml-hive-assist`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://cml-hive-assist.ai/install-cli.sh | bash -s -- --help
```
...
- Choose install method:
  - `npm` (default): `npm install -g cml-hive-assist@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `cml-hive-assist doctor --non-interactive` on upgrades and git installs (best effort).

Examples:

```powershell
iwr -useb https://cml-hive-assist.ai/install.ps1 | iex
```

```powershell
iwr -useb https://cml-hive-assist.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://cml-hive-assist.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\cml-hive-assist"
```
...
- **"cml-hive-assist" is not recognized**: your npm global bin folder is not on PATH. Most systems use
