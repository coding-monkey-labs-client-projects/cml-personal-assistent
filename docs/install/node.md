...
read_when:
  - "You installed CmlHiveAssist but `cml-hive-assist` is “command not found”"
...
CmlHiveAssist’s runtime baseline is **Node 22+**.

If you can run `npm install -g cml-hive-assist@latest` but later see `cml-hive-assist: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
