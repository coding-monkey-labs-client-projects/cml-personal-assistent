...
read_when:
  - You want to understand what “context” means in CmlHiveAssist
...
“Context” is **everything CmlHiveAssist sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (CmlHiveAssist-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How CmlHiveAssist builds the system prompt

The system prompt is **CmlHiveAssist-owned** and rebuilt each run. It includes:
...
By default, CmlHiveAssist injects a fixed set of workspace files (if present):
