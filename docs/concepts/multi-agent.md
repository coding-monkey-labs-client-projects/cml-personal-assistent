...
- **Session store** (chat history + routing state) under `~/.cml-hive-assist/agents/<agentId>/sessions`.
...
available from `~/.cml-hive-assist/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
If you do nothing, CmlHiveAssist runs a single agent:
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.cml-hive-assist/credentials/whatsapp/personal
