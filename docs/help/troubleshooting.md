...
### `docs.cml-hive-assist.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.cml-hive-assist.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.cml-hive-assist.ai` to the allowlist, then retry.
...
- Check the allowlist: `cml-hive-assist config get agents.defaults.models`
...
If you can, include the relevant log tail from `cml-hive-assist logs --follow`.
