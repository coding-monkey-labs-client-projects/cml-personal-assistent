---
summary: "All configuration options for ~/.cml-hive-assist/cml-hive-assist.json with examples"
read_when:
...
CmlHiveAssist reads an optional **JSON5** config from `~/.cml-hive-assist/cml-hive-assist.json` (comments + trailing commas allowed).

If the file is missing, CmlHiveAssist uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.cml-hive-assist/workspace`). You usually only need a config to:
...
CmlHiveAssist only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `cml-hive-assist doctor`, `cml-hive-assist logs`, `cml-hive-assist health`, `cml-hive-assist status`, `cml-hive-assist service`, `cml-hive-assist help`).
- Run `cml-hive-assist doctor` to see the exact issues.
- Run `cml-hive-assist doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `cml-hive-assist config set`. Keep a backup of `~/.cml-hive-assist/cml-hive-assist.json`.
...
```bash
cml-hive-assist gateway call config.get --params '{}' # capture payload.hash
cml-hive-assist gateway call config.apply --params '{
...
```bash
cml-hive-assist gateway call config.get --params '{}' # capture payload.hash
cml-hive-assist gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.cml-hive-assist/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@cml-hive-assist", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.cml-hive-assist/cml-hive-assist.json
{
  gateway: { port: 18789 },
...
```json5
// ~/.cml-hive-assist/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.cml-hive-assist/workspace" }],
}
```
...
{ "$include": "/etc/cml-hive-assist/base.json5" } // absolute
...
```json5
// ~/.cml-hive-assist/clients/mueller/agents.json5
[
...
```json5
// ~/.cml-hive-assist/clients/mueller/broadcast.json5
{
...
CmlHiveAssist reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, CmlHiveAssist runs your login shell and imports only the missing expected keys (never overrides).
...
CmlHiveAssist stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.cml-hive-assist/agents/<agentId>/agent/auth-profiles.json`)
...
- `~/.cml-hive-assist/agent/*` (migrated by `cml-hive-assist doctor` into `~/.cml-hive-assist/agents/<defaultAgentId>/agent/*`)
...
If set, CmlHiveAssist derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/cml-hive-assist/cml-hive-assist-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/cml-hive-assist/cml-hive-assist.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/cml-hive-assist/cml-hive-assist.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.cml-hive-assist/credentials/whatsapp/biz
          // authDir: "~/.cml-hive-assist/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `cml-hive-assist doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@cml-hive-assist", "cml-hive-assist"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@cml-hive-assist"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.cml-hive-assist/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.cml-hive-assist/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.cml-hive-assist/workspace-home" },
      { id: "work", workspace: "~/.cml-hive-assist/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `cml-hive-assist.json`).
...
CmlHiveAssist starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["cml-hive-assist-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
CmlHiveAssist starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `cml-hive-assist plugins install @cml-hive-assist/mattermost` (or `./extensions/mattermost` from a git checkout).
...
CmlHiveAssist starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
CmlHiveAssist spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, CmlHiveAssist
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/cml-hive-assist" } },
}
```
...
When a file exceeds this limit, CmlHiveAssist logs a warning and injects a truncated
...
message envelopes). If unset, CmlHiveAssist uses the host timezone at runtime.
...
`[cml-hive-assist]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[cml-hive-assist]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[cml-hive-assist]"`, CmlHiveAssist will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, CmlHiveAssist generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.cml-hive-assist/settings/tts.json",
      elevenlabs: {
...
CmlHiveAssist also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, CmlHiveAssist currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `cml-hive-assist configure --section web`, or use `BRAVE_API_KEY` env var)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.cml-hive-assist/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "cml-hive-assist-sandbox-browser:bookworm-slim",
          containerPrefix: "cml-hive-assist-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
CmlHiveAssist uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.cml-hive-assist/agents/<agentId>/agent/models.json` or by defining the same schema inside your
CmlHiveAssist config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, CmlHiveAssist writes/merges a `models.json` into
`~/.cml-hive-assist/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. CmlHiveAssist uses
...
- Shortcut: `cml-hive-assist onboard --auth-choice opencode-zen`.
...
Shortcut: `cml-hive-assist onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `cml-hive-assist onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `cml-hive-assist onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `cml-hive-assist onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `cml-hive-assist onboard --auth-choice minimax-api`.
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.cml-hive-assist/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.cml-hive-assist/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, CmlHiveAssist stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.cml-hive-assist/skills` (workspace skills
...
from `~/.cml-hive-assist/extensions`, `<workspace>/.cml-hive-assist/extensions`, plus any
...
### `browser` (cml-hive-assist-managed browser)

CmlHiveAssist can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for cml-hive-assist and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (CmlHiveAssist.app menubar, or `cml-hive-assist gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/cml-hive-assist" }
...
- Examples: `"/ui"`, `"/cml-hive-assist"`, `"/apps/cml-hive-assist"`.
...
- When a connection comes from one of these IPs, CmlHiveAssist uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- CmlHiveAssist.app watches `~/.cml-hive-assist/cml-hive-assist.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
- `cml-hive-assist --dev …` → uses `~/.cml-hive-assist-dev` + shifts ports from base `19001`
- `cml-hive-assist --profile <name> …` → uses `~/.cml-hive-assist-<name>` (port via config/env/flags)
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.cml-hive-assist/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
- `x-cml-hive-assist-token: <token>` **or**
- `?token=<token>`
...
Gmail helper config (used by `cml-hive-assist webhooks gmail setup` / `run`):
...
Note: when `tailscale.mode` is on, CmlHiveAssist defaults `serve.path` to `/` so
...
Default root: `~/.cml-hive-assist/workspace/canvas`  
Default port: `18793` (chosen to avoid the cml-hive-assist browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__cml-hive-assist__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__cml-hive-assist__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.cml-hive-assist/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.cml-hive-assist/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.cml-hive-assist/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.cml-hive-assist/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_cml-hive-assist-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_cml-hive-assist-gw._tcp` under `~/.cml-hive-assist/dns/` using the configured discovery domain (example: `cml-hive-assist.internal.`).
