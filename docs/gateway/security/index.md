...
## Quick check: `cml-hive-assist security audit`
...
- Tighten local perms (`~/.cml-hive-assist` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

CmlHiveAssist is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, CmlHiveAssist also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.cml-hive-assist/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.cml-hive-assist/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.cml-hive-assist/credentials/oauth.json`
...
`cml-hive-assist security audit` warns when this setting is enabled.
...
CmlHiveAssist stores session transcripts on disk under `~/.cml-hive-assist/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.cml-hive-assist` (see the audit section below). If you need
...
CmlHiveAssist can refresh the skills list mid-session:
...
By default, CmlHiveAssist routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
CmlHiveAssist has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.cml-hive-assist/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.cml-hive-assist or your logs.”
...
4. **Re-run audit**
   - `cml-hive-assist security audit --deep` and confirm the report is clean.
...
- `~/.cml-hive-assist/cml-hive-assist.json`: `600` (user read/write only)
- `~/.cml-hive-assist`: `700` (user only)

`cml-hive-assist doctor` can warn and offer to tighten these permissions.
...
The Gateway broadcasts its presence via mDNS (`_cml-hive-assist-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `cml-hive-assist doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), CmlHiveAssist
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. CmlHiveAssist verifies the identity by resolving the
...
- CmlHiveAssist will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- When sharing diagnostics, prefer `cml-hive-assist status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.cml-hive-assist/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `cml-hive-assist` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only CmlHiveAssist clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `cml-hive-assist gateway` process.
...
1. Check Gateway logs: `/tmp/cml-hive-assist/cml-hive-assist-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.cml-hive-assist/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + CmlHiveAssist version
...
Found a vulnerability in CmlHiveAssist? Please report responsibly:

1. Email: security@cml-hive-assist.ai
