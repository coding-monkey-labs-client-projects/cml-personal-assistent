...
CmlHiveAssist uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
2. Publish DNS‑SD records for `_cml-hive-assist-gw._tcp` under a dedicated zone
   (example: `cml-hive-assist.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

CmlHiveAssist supports any discovery domain; `cml-hive-assist.internal.` is just an example.
...
- serve your chosen domain (example: `cml-hive-assist.internal.`) from `~/.cml-hive-assist/dns/<domain>.db`
...
`_cml-hive-assist-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.cml-hive-assist/cml-hive-assist.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_cml-hive-assist-gw._tcp`.

## Service types

- `_cml-hive-assist-gw._tcp` — gateway transport beacon (used by macOS/iOS/Android nodes).
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `cml-hive-assist` entrypoint)
...
The iOS node uses `NWBrowser` to discover `_cml-hive-assist-gw._tcp`.
