...
cml-hive-assist nodes reject <requestId>
cml-hive-assist nodes status
cml-hive-assist nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.cml-hive-assist`):
