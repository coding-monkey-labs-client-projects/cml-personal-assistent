...
`cml-hive-assist doctor` is the repair + migration tool for CmlHiveAssist. It fixes stale
...
- Extra workspace dir detection (`~/cml-hive-assist`).
...
you to run `cml-hive-assist doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.cml-hive-assist/cml-hive-assist.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.cml-hive-assist/sessions/` to `~/.cml-hive-assist/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.cml-hive-assist/agent/` to `~/.cml-hive-assist/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.cml-hive-assist/credentials/*.json` (except `oauth.json`)
  - to `~/.cml-hive-assist/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `cml-hive-assist doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.cml-hive-assist` folders exist across
  home directories or when `CML_HIVE_ASSIST_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.cml-hive-assist/cml-hive-assist.json` is
...
offers to remove them and install the CmlHiveAssist service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named CmlHiveAssist gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `cml-hive-assist doctor --generate-gateway-token` to force token
...
- `cml-hive-assist doctor` prompts before rewriting supervisor config.
- `cml-hive-assist doctor --yes` accepts the default repair prompts.
- `cml-hive-assist doctor --repair` applies recommended fixes without prompts.
- `cml-hive-assist doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `cml-hive-assist gateway install --force`.
