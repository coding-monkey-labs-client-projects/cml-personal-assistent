...
CmlHiveAssist supports OAuth and API keys for model providers. For Anthropic
...
2. Put it on the **gateway host** (the machine running `cml-hive-assist gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.cml-hive-assist/.env` so the daemon can read it:
...
API keys for daemon use: `cml-hive-assist onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.cml-hive-assist/.env`, systemd/launchd).
...
Then paste it into CmlHiveAssist:
...
Run `cml-hive-assist models status` to confirm which profile is expiring. If the profile
