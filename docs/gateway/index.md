...
- Replaces the legacy `gateway` command. CLI entry point: `cml-hive-assist gateway`.
...
- macOS: `bot.molt.<profile>` (legacy `com.cml-hive-assist.*` may still exist)
- Linux: `cml-hive-assist-gateway-<profile>.service`
- Windows: `CmlHiveAssist Gateway (<profile>)`
...
- `health` — full health snapshot (same shape as `cml-hive-assist health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `cml-hive-assist`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `cml-hive-assist gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.cml-hive-assist.*` is cleaned up).
  - `cml-hive-assist doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are CmlHiveAssist profile services.
  ...
  - Cleanup: `cml-hive-assist gateway uninstall` (current service) and `cml-hive-assist doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `cml-hive-assist gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- CmlHiveAssist.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.cml-hive-assist.*` labels still unload cleanly).
- To stop it cleanly, use `cml-hive-assist gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `cml-hive-assist gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `cml-hive-assist gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

CmlHiveAssist installs a **systemd user service** by default on Linux/WSL2. We
...
`cml-hive-assist gateway install` writes the user unit. `cml-hive-assist doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/cml-hive-assist-gateway[-<profile>].service`:

```
[Unit]
Description=CmlHiveAssist Gateway (profile: <profile>, v<version>)
...
Create `/etc/systemd/system/cml-hive-assist-gateway[-<profile>].service` (copy the unit above,
...
- `cml-hive-assist gateway health|status` — request health/status over the Gateway WS.
- `cml-hive-assist message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `cml-hive-assist agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `cml-hive-assist gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `cml-hive-assist gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `cml-hive-assist gateway` and the legacy TCP control port.
