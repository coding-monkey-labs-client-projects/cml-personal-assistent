---
summary: "Quick troubleshooting guide for common CmlHiveAssist failures"
read_when:
...
When CmlHiveAssist misbehaves, here's how to fix it.
...
| `cml-hive-assist gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `cml-hive-assist channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
...
**No config file yet?** Run `cml-hive-assist setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.cml-hive-assist/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `cml-hive-assist doctor`
...
- then run `cml-hive-assist sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `cml-hive-assist gateway` and the service.
...
**If `cml-hive-assist gateway status` shows a config mismatch**
...
- Fix: run `cml-hive-assist doctor` to update it (or `cml-hive-assist gateway install --force` for a full rewrite).
...
If you upgraded from older installs, you might still have `~/cml-hive-assist` on disk.
...
Symptoms: `pwd` or file tools show `~/.cml-hive-assist/sandboxes/...` even though you
...
CmlHiveAssist intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `cml-hive-assist models list` or
  `cml-hive-assist models scan` and choose a supported one.
...
- ❌ `@cml-hive-assist` + image
- ✅ `@cml-hive-assist check this` + image
...
CmlHiveAssist keeps conversation history in memory.
...
CmlHiveAssist now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `cml-hive-assist doctor` reports every invalid entry.
- `cml-hive-assist doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `cml-hive-assist logs`, `cml-hive-assist health`, `cml-hive-assist status`, `cml-hive-assist gateway status`, and `cml-hive-assist gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/cml-hive-assist/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `cml-hive-assist models status` (CLI).
...
5. Run `cml-hive-assist channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. CmlHiveAssist scrubs/normalizes tool
schemas in current `main`, but the fix is not in the last release yet (as of
January 13, 2026).

Fix checklist:

1. **Update CmlHiveAssist**:
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/cml-hive-assist/cml-hive-assist/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.cml-hive-assist.* still works)
```
...
Ensure the global `cml-hive-assist` CLI is installed and matches the app version:
...
1. Check logs first: `/tmp/cml-hive-assist/` (default: `cml-hive-assist-YYYY-MM-DD.log`, or your configured `logging.file`)
2. Search existing issues on GitHub
3. Open a new issue with:
   - CmlHiveAssist version
