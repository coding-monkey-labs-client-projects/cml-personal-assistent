---
summary: "Use MiniMax M2.1 in CmlHiveAssist"
read_when:
  - You want MiniMax models in CmlHiveAssist
...
```bash
cml-hive-assist plugins enable minimax-portal-auth  # skip if already loaded.
cml-hive-assist gateway restart  # restart if gateway is already running
cml-hive-assist onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/cml-hive-assist/cml-hive-assist/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `cml-hive-assist.json`:
...
- Use `cml-hive-assist models list` and `cml-hive-assist models set minimax/MiniMax-M2.1` to switch.
...
- Running `cml-hive-assist configure` and selecting **MiniMax M2.1**, or
