---
summary: "Use Venice AI privacy-focused models in CmlHiveAssist"
read_when:
  - You want privacy-focused inference in CmlHiveAssist
...
```bash
cml-hive-assist onboard --auth-choice venice-api-key
```
...
```bash
cml-hive-assist chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, CmlHiveAssist shows all available Venice models. Pick based on your needs:
...
```bash
cml-hive-assist models set venice/claude-opus-45
cml-hive-assist models set venice/llama-3.3-70b
```

List all available models:

```bash
cml-hive-assist models list | grep venice
```

## Configure via `cml-hive-assist configure`

1. Run `cml-hive-assist configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
CmlHiveAssist automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
cml-hive-assist chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
cml-hive-assist chat --model venice/claude-opus-45

# Use uncensored model
cml-hive-assist chat --model venice/venice-uncensored

# Use vision model with image
cml-hive-assist chat --model venice/qwen3-vl-235b-a22b

# Use coding model
cml-hive-assist chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `cml-hive-assist models list` to see currently available models. Some models may be temporarily offline.
