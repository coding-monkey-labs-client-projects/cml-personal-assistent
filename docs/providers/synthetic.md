---
summary: "Use Synthetic's Anthropic-compatible API in CmlHiveAssist"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. CmlHiveAssist registers it as the
...
Note: CmlHiveAssist's Anthropic client appends `/v1` to the base URL, so use
