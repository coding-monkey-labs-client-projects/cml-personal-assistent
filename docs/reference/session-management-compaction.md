...
This document explains how CmlHiveAssist manages sessions end-to-end:
...
CmlHiveAssist is designed around a single **Gateway process** that owns session state.
...
CmlHiveAssist persists sessions in two layers:
...
- Transcripts: `~/.cml-hive-assist/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

CmlHiveAssist resolves these via `src/config/sessions.ts`.
...
CmlHiveAssist intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (CmlHiveAssist consumes the events, but Pi decides when to compact).
...
CmlHiveAssist also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, CmlHiveAssist bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, CmlHiveAssist leaves it alone.
...
- `cml-hive-assist status` (CLI)
- `cml-hive-assist sessions` / `sessions --json`
...
CmlHiveAssist supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- CmlHiveAssist strips/suppresses this in the delivery layer.

As of `2026.1.10`, CmlHiveAssist also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
CmlHiveAssist uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but CmlHiveAssist’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `cml-hive-assist status`.
