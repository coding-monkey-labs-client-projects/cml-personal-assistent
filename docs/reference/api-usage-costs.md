...
CmlHiveAssist features that can generate provider usage or paid API calls.
...
- `cml-hive-assist status --usage` and `cml-hive-assist channels list` show provider **usage windows**
...
CmlHiveAssist can pick up credentials from:
...
- `cml-hive-assist models status --json`
...
`cml-hive-assist models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
