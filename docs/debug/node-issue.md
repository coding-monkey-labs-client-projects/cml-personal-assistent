...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in CmlHiveAssist
...
Running CmlHiveAssist via Node with `tsx` fails at startup with:

```
[cml-hive-assist] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `cml-hive-assist status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch cml-hive-assist.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node cml-hive-assist.mjs status` works on Node 25.
