---
summary: "Direct `cml-hive-assist agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `cml-hive-assist agent` (direct agent runs)

`cml-hive-assist agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `cml-hive-assist message --target`).
