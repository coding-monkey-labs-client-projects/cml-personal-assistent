...
- **gateway host** → `cml-hive-assist` process on the gateway machine
...
`~/.cml-hive-assist/exec-approvals.json` directly.

CLI: `cml-hive-assist approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
