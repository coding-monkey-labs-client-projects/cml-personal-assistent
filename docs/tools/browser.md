...
read_when:
  ...
  - Debugging why cml-hive-assist is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (CmlHiveAssist-managed)"
---

# Browser (cml-hive-assist-managed)

CmlHiveAssist can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `cml-hive-assist` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `cml-hive-assist` for the isolated managed browser.

## What you get

- A separate browser profile named **cml-hive-assist** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`cml-hive-assist`, `work`, `remote`, ...).
...
## Profiles: `cml-hive-assist` vs `chrome`

- `cml-hive-assist`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the CmlHiveAssist
  extension to be attached to a tab).

Set `browser.defaultProfile: "cml-hive-assist"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.cml-hive-assist/cml-hive-assist.json`.
...
- If you override the Gateway port (`gateway.port` or `CML_HIVE_ASSIST_GATEWAY_PORT`),
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "cml-hive-assist"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `cml-hive-assist` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.

## Use Brave (or another Chromium-based browser)

If your **system default** browser is Chromium-based (Chrome/Brave/Edge/etc),
CmlHiveAssist uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, CmlHiveAssist will not launch a local browser.
...
CmlHiveAssist preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, CmlHiveAssist can
...
CDP endpoints over HTTPS. You can point a CmlHiveAssist browser profile at a
...
CmlHiveAssist supports multiple named profiles (routing configs). Profiles can be:

- **cml-hive-assist-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:

- The `cml-hive-assist` profile is auto-created if missing.
...
CmlHiveAssist can also drive **your existing Chrome tabs** (no separate “cml-hive-assist” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **CmlHiveAssist Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
- “Load unpacked” → select the directory printed by `cml-hive-assist browser extension path`
...
When launching locally, CmlHiveAssist picks the first available:
...
error. ARIA snapshots and basic screenshots still work for cml-hive-assist-managed Chrome.
For the Chrome extension relay driver, ARIA snapshots and screenshots require Playwright.

If you see `Playwright is not available in this gateway build`, install the full
Playwright package (not `playwright-core`) and restart the gateway, or reinstall
CmlHiveAssist with browser support.
...
```bash
docker compose run --rm cml-hive-assist-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`CML_HIVE_ASSIST_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `snapshot`:
  ...
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-cml-hive-assist-managed-browser)).
...
CmlHiveAssist supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `cml-hive-assist browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `cml-hive-assist browser click 12`, `cml-hive-assist browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `cml-hive-assist browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `cml-hive-assist browser click e12`, `cml-hive-assist browser highlight e12`.
...
3. If it still fails: `cml-hive-assist browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `cml-hive-assist browser trace start`
   - reproduce the issue
   - `cml-hive-assist browser trace stop` (prints `TRACE:<path>`)
...
- The cml-hive-assist browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `cml-hive-assist browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (cml-hive-assist, chrome, or remote CDP).
