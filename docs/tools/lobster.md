---
title: Lobster
summary: "Typed workflow runtime for CmlHiveAssist with resumable approval gates."
...
Lobster is a workflow shell that lets CmlHiveAssist run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: CmlHiveAssist runs one Lobster tool call and gets a structured result.
...
CmlHiveAssist launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In CmlHiveAssist tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the CmlHiveAssist Gateway (see the [Lobster repo](https://github.com/cml-hive-assist/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), CmlHiveAssist keeps core tools enabled. To restrict core
...
→ cml-hive-assist calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ cml-hive-assist calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls CmlHiveAssist tools that do.
