---
summary: "Chrome extension: let CmlHiveAssist drive your existing Chrome tab"
read_when:
...
The CmlHiveAssist Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate cml-hive-assist-managed Chrome profile.
...
CmlHiveAssist then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the CmlHiveAssist release (npm package) as static files. There is no separate “build” step.

After upgrading CmlHiveAssist:

- Re-run `cml-hive-assist browser extension install` to refresh the installed files under your CmlHiveAssist state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

CmlHiveAssist ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want CmlHiveAssist to control.
...
- `ON`: attached; CmlHiveAssist can drive that tab.
...
Debugging: `cml-hive-assist sandbox explain`
...
`cml-hive-assist browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `cml-hive-assist browser extension install` first to copy the extension to a stable location under your CmlHiveAssist state directory.
...
- **This is not isolated** like the dedicated cml-hive-assist-managed profile.
