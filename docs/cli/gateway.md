---
summary: "CmlHiveAssist Gateway CLI (`cml-hive-assist gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is CmlHiveAssist’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `cml-hive-assist gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.cml-hive-assist/cml-hive-assist.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
- `--token <token>`: token override (also sets `CML_HIVE_ASSIST_GATEWAY_TOKEN` for the process).
- `--password <password>`: password override (also sets `CML_HIVE_ASSIST_GATEWAY_PASSWORD` for the process).
...
`gateway discover` scans for Gateway beacons (`_cml-hive-assist-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `cml-hive-assist.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
