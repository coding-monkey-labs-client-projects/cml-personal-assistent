---
summary: "CLI reference for `cml-hive-assist hooks` (agent hooks)"
read_when:
...
Enable a specific hook by adding it to your config (`~/.cml-hive-assist/config.json`).

**Note:** Hooks managed by plugins show `plugin:<id>` in `cml-hive-assist hooks list` and
...
- Copies the hook pack into `~/.cml-hive-assist/hooks/<id>`
...
**Output:** `~/.cml-hive-assist/workspace/memory/YYYY-MM-DD-slug.md`
