...
CmlHiveAssist can run agents in isolated Docker containers for security. The `sandbox` commands help you manage these containers, especially after updates or configuration changes.
...
cml-hive-assist sandbox recreate --session main       # Specific session
cml-hive-assist sandbox recreate --agent mybot        # Specific agent
cml-hive-assist sandbox recreate --browser            # Only browser containers
...
**Solution:** Use `cml-hive-assist sandbox recreate` to force removal of old containers. They'll be recreated automatically with current settings when next needed.

Tip: prefer `cml-hive-assist sandbox recreate` over manual `docker rm`. It uses the
Gateway’s container naming and avoids mismatches when scope/session keys change.

## Configuration

Sandbox settings live in `~/.cml-hive-assist/cml-hive-assist.json` under `agents.defaults.sandbox` (per-agent overrides go in `agents.list[].sandbox`):
