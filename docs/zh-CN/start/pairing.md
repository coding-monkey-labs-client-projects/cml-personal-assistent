...
"配对"是 CmlHiveAssist 的显式**所有者审批**步骤。
它用于两个场景：
...
存储在 `~/.cml-hive-assist/credentials/` 下：
...
- 旧版 `node.pair.*` API（CLI：`cml-hive-assist nodes pending/approve`）是一个独立的 Gateway网关所拥有的配对存储。WebSocket 节点仍然需要设备配对。
