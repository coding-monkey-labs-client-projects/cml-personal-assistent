...
- 随时重新打开：`cml-hive-assist dashboard`（复制链接，如可能则打开浏览器，无头环境下显示 SSH 提示）。
- 令牌保留在本地（仅作为查询参数）；UI 在首次加载后会移除它并保存到 localStorage 中。

## 令牌基础（本地 vs 远程）

- **Localhost**：打开 `http://127.0.0.1:18789/`。如果看到"unauthorized"，运行 `cml-hive-assist dashboard` 并使用带令牌的链接（`?token=...`）。
...
- 运行 `cml-hive-assist dashboard` 获取新的带令牌链接。
- 确保 Gateway网关可达（本地：`cml-hive-assist status`；远程：SSH 隧道 `ssh -N -L 18789:127.0.0.1:18789 user@host`，然后打开 `http://127.0.0.1:18789/?token=...`）。
