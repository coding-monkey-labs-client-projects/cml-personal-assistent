...
有关调试"为什么被阻止了？"，请参阅[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated)和 `cml-hive-assist sandbox explain`。
...
旧版 `agent.*` 配置会由 `cml-hive-assist doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
