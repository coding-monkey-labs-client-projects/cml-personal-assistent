...
本指南总结了在 CmlHiveAssist 中进行 Pi 集成开发的合理工作流。
...
- 以开发模式运行 Gateway网关：
  - `pnpm gateway:dev`
- 直接触发智能体：
  - `pnpm cml-hive-assist agent --message "Hello" --thinking low`
- 使用 TUI 进行交互式调试：
  - `pnpm tui`
...
状态存储在 CmlHiveAssist 状态目录下。默认为 `~/.cml-hive-assist`。如果设置了 `OPENCLAW_STATE_DIR`，则使用该目录。

要重置所有内容：

- `cml-hive-assist.json` 用于配置
...
- https://docs.cml-hive-assist.ai/testing
- https://docs.cml-hive-assist.ai/start/getting-started
