...
summary: 针对 CmlHiveAssist 最高风险路径的机器检查安全模型。
title: 形式化验证（安全模型）
x-i18n:
...
**目标（北极星）：** 提供机器检查的论证，证明 CmlHiveAssist 在明确的假设条件下能够执行其预期的安全策略（授权、会话隔离、工具门控和错误配置安全性）。
...
**尚未实现的目标：** 证明"CmlHiveAssist 在所有方面都是安全的"，或证明完整的 TypeScript 实现是正确的。
