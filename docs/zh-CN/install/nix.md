...
read_when:
  ...
  - 你想要一切固定且声明式管理
summary: 使用 Nix 声明式安装 CmlHiveAssist
title: Nix
x-i18n:
...
使用 Nix 运行 CmlHiveAssist 的推荐方式是通过 **[nix-cml-hive-assist](https://github.com/cml-hive-assist/nix-cml-hive-assist)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/cml-hive-assist/nix-cml-hive-assist](https://github.com/cml-hive-assist/nix-cml-hive-assist)**
>
> nix-cml-hive-assist 仓库是 Nix 安装的权威来源。本页仅为简要概览。
...
当设置了 `OPENCLAW_NIX_MODE=1` 时（nix-cml-hive-assist 会自动设置）：

CmlHiveAssist 支持 **Nix 模式**，使配置具有确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
CmlHiveAssist 从 `OPENCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `OPENCLAW_STATE_DIR` 中。

- `OPENCLAW_STATE_DIR`（默认：`~/.cml-hive-assist`）
- `OPENCLAW_CONFIG_PATH`（默认：`$OPENCLAW_STATE_DIR/cml-hive-assist.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/cml-hive-assist/cml-hive-assist/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段
（bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这使得 plist 对 SwiftPM
打包和 Nix 构建保持确定性（它们不依赖完整的 Xcode 工具链）。

## 相关内容

- [nix-cml-hive-assist](https://github.com/cml-hive-assist/nix-cml-hive-assist) — 完整设置指南
