---
read_when:
  - 你正在将 CmlHiveAssist 迁移到新的笔记本/服务器
  - 你想保留会话、认证和渠道登录状态（WhatsApp 等）
summary: 将 CmlHiveAssist 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
# 将 CmlHiveAssist 迁移到新机器

本指南介绍如何将 CmlHiveAssist Gateway网关从一台机器迁移到另一台，**无需重新进行新手引导**。
...
- `~/.cml-hive-assist/workspace/`（推荐工作区）
- 你创建的自定义文件夹
...
- Gateway网关配置（`cml-hive-assist.json`）
...
如果你有多个配置文件/状态目录（例如 `~/.cml-hive-assist-main`、`~/.cml-hive-assist-work`），请分别归档。

### 步骤 1 — 在新机器上安装 CmlHiveAssist

在**新**机器上安装 CLI（如有需要也安装 Node）：

- 参见：[安装](/install)

在此阶段，新手引导创建一个全新的 `~/.cml-hive-assist/` 是没问题的 — 你将在下一步覆盖它。
...
- `cml-hive-assist status` 显示 Gateway网关正在运行
...
- [CmlHiveAssist 将数据存储在哪里？](/help/faq#where-does-cml-hive-assist-store-its-data)
