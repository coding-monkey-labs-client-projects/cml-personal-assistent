...
read_when:
  - 安装 CmlHiveAssist
  - 你想从 GitHub 安装
summary: 安装 CmlHiveAssist（推荐安装器、全局安装或从源码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `cml-hive-assist` 并运行新手引导。
...
pnpm approve-builds -g                # 批准 cml-hive-assist、node-llama-cpp、sharp 等
pnpm add -g cml-hive-assist@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果尚未全局安装，可通过 `pnpm cml-hive-assist ...` 运行仓库命令。
...
- 运行新手引导：`cml-hive-assist onboard --install-daemon`
- 快速检查：`cml-hive-assist doctor`
- 检查 Gateway网关健康状态：`cml-hive-assist status` + `cml-hive-assist health`
- 打开仪表盘：`cml-hive-assist dashboard`
...
- `--git-dir <path>`（默认：`~/cml-hive-assist`）
...
## 故障排除：找不到 `cml-hive-assist`（PATH 问题）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在** `echo "$PATH"` 的输出中，说明你的 shell 无法找到全局 npm 二进制文件（包括 `cml-hive-assist`）。
