---
read_when:
  - 你想在 CmlHiveAssist 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指导
summary: 在 CmlHiveAssist 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
cml-hive-assist plugins enable minimax-portal-auth  # 如已加载则跳过
cml-hive-assist gateway restart  # 如 Gateway网关已在运行则重启
...
详情请参阅 [MiniMax OAuth 插件 README](https://github.com/cml-hive-assist/cml-hive-assist/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `cml-hive-assist models list` 和 `cml-hive-assist models set minimax/MiniMax-M2.1` 进行切换。
...
- 运行 `cml-hive-assist configure` 并选择 **MiniMax M2.1**，或
