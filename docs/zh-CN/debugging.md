...
当你需要切换一些不常用的设置而又不想编辑 `cml-hive-assist.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.cml-hive-assist-dev`，并将 Gateway网关默认端口设为 `19001`（派生端口随之偏移）。
...
如果你还没有全局安装，可通过 `pnpm cml-hive-assist ...` 运行 CLI。
...
CmlHiveAssist 可以在任何过滤/格式化之前记录**原始助手流**。
