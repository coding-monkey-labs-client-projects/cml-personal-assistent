---
summary: "Gmail Pub/Sub push wired into CmlHiveAssist webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to CmlHiveAssist
...
Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> CmlHiveAssist webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- CmlHiveAssist hooks enabled (see [Webhooks](/automation/webhook)).
...
```json5
{
  hooks: {
    enabled: true,
    token: "CML_HIVE_ASSIST_HOOK_TOKEN",
    path: "/hooks",
    presets: ["gmail"],
  },
}
...
Use the CmlHiveAssist helper to wire everything together (installs deps on macOS via brew):
...
- Writes `hooks.gmail` config for `cml-hive-assist webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, CmlHiveAssist automatically sets
...
- Set `CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
gog gmail watch serve \
  ...
  --hook-token CML_HIVE_ASSIST_HOOK_TOKEN \
  --include-body \
  --max-bytes 20000
```
...
- `--hook-url` points to CmlHiveAssist `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to CmlHiveAssist.

Recommended: `cml-hive-assist webhooks gmail run` wraps the same flow and auto-renews the watch.
