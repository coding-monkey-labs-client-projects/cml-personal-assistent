...
Deploy CML Hive Assist on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/cml-hive-assist/cml-hive-assist"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run CML Hive Assist:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CML_HIVE_ASSIST_STATE_DIR
        value: /data/.cml-hive-assist
      - key: CML_HIVE_ASSIST_WORKSPACE_DIR
        value: /data/workspace
      - key: CML_HIVE_ASSIST_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: cml-hive-assist-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/cml-hive-assist`.
...
If you use the original CML Hive Assist repository, Render will not auto-deploy your CML Hive Assist. To update it, run a manual Blueprint sync from the dashboard.
...
For CML Hive Assist, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any CML Hive Assist host.
