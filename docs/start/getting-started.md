...
Fastest chat: open the Control UI (no channel setup needed). Run `cml-hive-assist dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`cml-hive-assist onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `cml-hive-assist configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.cml-hive-assist/credentials/oauth.json`
...
If you’re hacking on CmlHiveAssist itself, run from source:
...
```bash
node cml-hive-assist.mjs gateway --port 18789 --verbose
```
...
If `cml-hive-assist health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `cml-hive-assist status --all` is the best pasteable, read-only debug report.
Health probes: `cml-hive-assist health` (or `cml-hive-assist status --deep`) asks the running gateway for a health snapshot.
