---
summary: "First-run onboarding flow for CmlHiveAssist (macOS app)"
read_when:
...
- Writes credentials to `~/.cml-hive-assist/credentials/oauth.json`
...
The app can install the global `cml-hive-assist` CLI via npm/pnpm so terminal
...
On the first agent run, CmlHiveAssist bootstraps a workspace (default `~/.cml-hive-assist/workspace`):
