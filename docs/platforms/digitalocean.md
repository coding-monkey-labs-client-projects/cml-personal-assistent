---
summary: "CmlHiveAssist on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up CmlHiveAssist on DigitalOcean
  - Looking for cheap VPS hosting for CmlHiveAssist
title: "DigitalOcean"
---

# CmlHiveAssist on DigitalOcean

## Goal

Run a persistent CmlHiveAssist Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status cml-hive-assist-gateway.service

# View logs
journalctl --user -u cml-hive-assist-gateway.service -f
```
...
```bash
cml-hive-assist config set gateway.bind tailnet
cml-hive-assist gateway restart
```
...
```bash
cml-hive-assist channels login whatsapp
# Scan QR code
```
...
- `~/.cml-hive-assist/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u cml-hive-assist --no-pager -n 50
```
