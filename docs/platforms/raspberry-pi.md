---
summary: "CmlHiveAssist on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up CmlHiveAssist on a Raspberry Pi
  - Running CmlHiveAssist on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# CmlHiveAssist on Raspberry Pi

## Goal

Run a persistent, always-on CmlHiveAssist Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u cml-hive-assist -f
```
...
Most CmlHiveAssist features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled cml-hive-assist

# Enable if not
sudo systemctl enable cml-hive-assist

# Start on boot
sudo systemctl start cml-hive-assist
```
...
```bash
# Check logs
journalctl -u cml-hive-assist --no-pager -n 100

# Common fix: rebuild
cd ~/cml-hive-assist  # if using hackable install
npm run build
sudo systemctl restart cml-hive-assist
```
