...
**Goal:** CmlHiveAssist Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
```toml
app = "my-cml-hive-assist"  # Your app name
primary_region = "iad"
...
[env]
  NODE_ENV = "production"
  CML_HIVE_ASSIST_PREFER_PNPM = "1"
  CML_HIVE_ASSIST_STATE_DIR = "/data"
...
| `internal_port = 3000`                | Must match `--port 3000` (or `CML_HIVE_ASSIST_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`                   | 512MB is too small; 2GB recommended                                                |
| `CML_HIVE_ASSIST_STATE_DIR = "/data"` | Persists state on the volume                                                       |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set CML_HIVE_ASSIST_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `CML_HIVE_ASSIST_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `cml-hive-assist.json` where they could be accidentally exposed or logged.
...
**Note:** With `CML_HIVE_ASSIST_STATE_DIR=/data`, the config path is `/data/cml-hive-assist.json`.
...
Or visit `https://my-cml-hive-assist.fly.dev/`

Paste your gateway token (the one from `CML_HIVE_ASSIST_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `CML_HIVE_ASSIST_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/cml-hive-assist.json` should be read on restart.
...
**Fix:** Ensure `CML_HIVE_ASSIST_STATE_DIR=/data` is set in `fly.toml` and redeploy.
