...
service via the external `cml-hive-assist` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`CML_HIVE_ASSIST_PROFILE`; legacy `com.cml-hive-assist.*` is supported).
...
- Writes `~/.cml-hive-assist/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.cml-hive-assist/disable-launchagent`,
