...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `cml-hive-assist.internal.`), **or**
...
The Gateway advertises `_cml-hive-assist-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `cml-hive-assist.internal.`) and Tailscale split DNS.
...
- The Gateway canvas host serves `/__cml-hive-assist__/canvas/` and `/__cml-hive-assist__/a2ui/`.
...
- Pairing prompt never appears: run `cml-hive-assist nodes pending` and approve manually.
