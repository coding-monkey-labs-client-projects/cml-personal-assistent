...
CmlHiveAssist supports **camera capture** for agent workflows:
...
```bash
cml-hive-assist nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
...
- **Settings → General → Allow Camera** (`cml-hive-assist.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `cml-hive-assist` CLI to invoke camera commands on the macOS node.

Examples:

```bash
cml-hive-assist nodes camera list --node <id>            # list camera ids
cml-hive-assist nodes camera snap --node <id>            # prints MEDIA:<path>
...
cml-hive-assist nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
cml-hive-assist nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
...
- `cml-hive-assist nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
cml-hive-assist nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
