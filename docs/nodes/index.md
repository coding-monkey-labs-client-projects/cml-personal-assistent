...
macOS can also run in **node mode**: the menubar app connects to the Gateway’s WS server and exposes its local canvas/camera commands as a node (so `cml-hive-assist nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `cml-hive-assist nodes pending/approve/reject`) is a separate gateway-owned
...
- **Approvals**: enforced on the node host via `~/.cml-hive-assist/exec-approvals.json`.
...
- `--display-name` on `cml-hive-assist node run` / `cml-hive-assist node install` (persists in `~/.cml-hive-assist/node.json` on the node).
- `cml-hive-assist nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
Approvals live on the node host at `~/.cml-hive-assist/exec-approvals.json`.
...
cml-hive-assist nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
cml-hive-assist nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.cml-hive-assist/exec-approvals.json`).
...
CmlHiveAssist can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.cml-hive-assist/node.json`.
...
- The macOS menubar app connects to the Gateway WS server as a node (so `cml-hive-assist nodes …` works against this Mac).
