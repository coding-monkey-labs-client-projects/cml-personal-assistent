...
This is handy when you need to toggle obscure settings without editing `cml-hive-assist.json`.
...
- **Global `--dev` (profile):** isolates state under `~/.cml-hive-assist-dev` and
...
## Raw stream logging (CmlHiveAssist)

CmlHiveAssist can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.cml-hive-assist/logs/raw-stream.jsonl
```
...
`~/.cml-hive-assist/logs/raw-stream.jsonl`
