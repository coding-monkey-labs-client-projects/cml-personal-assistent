...
INSTALL_URL="${CML_HIVE_ASSIST_INSTALL_URL:-https://cml-hive-assist.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CML_HIVE_ASSIST_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CML_HIVE_ASSIST_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="cml-hive-assist"
PACKAGE_NAME="${CML_HIVE_ASSIST_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CML_HIVE_ASSIST_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CML_HIVE_ASSIST_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CML_HIVE_ASSIST_INSTALL_LATEST_OUT:-}"
fi
