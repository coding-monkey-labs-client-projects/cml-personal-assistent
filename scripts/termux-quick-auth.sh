...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/cml-hive-assist/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "CmlHiveAssist Re-Auth" -c "After getting key, run: ssh $SERVER '~/cml-hive-assist/scripts/mobile-reauth.sh'" --id cml-hive-assist-auth
        ;;
    *)
