#!/usr/bin/env bash
# Reset CmlHiveAssist like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CML_HIVE_ASSIST_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="CmlHiveAssist.app/Contents/MacOS/CmlHiveAssist"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/CmlHiveAssist"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/CmlHiveAssist"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/CmlHiveAssist"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.cml-hive-assist.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/cml-hive-assist-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CML_HIVE_ASSIST_RESTART_LOG:-/tmp/cml-hive-assist-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CML_HIVE_ASSIST_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.cml-hive-assist/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CML_HIVE_ASSIST_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node cml-hive-assist.mjs daemon install --force --runtime node"
      log "  node cml-hive-assist.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.cml-hive-assist/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.cml-hive-assist.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing CmlHiveAssist instances"
kill_all_cml-hive-assist
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product CmlHiveAssist"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/CmlHiveAssist.app" ]]; then
    APP_BUNDLE="/Applications/CmlHiveAssist.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/CmlHiveAssist.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/CmlHiveAssist.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/CmlHiveAssist.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CML_HIVE_ASSIST_APP_BUNDLE to your installed CmlHiveAssist.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node cml-hive-assist.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node cml-hive-assist.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".cml-hive-assist", "cml-hive-assist.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: CmlHiveAssist is running."
else
