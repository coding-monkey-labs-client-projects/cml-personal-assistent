...
#
# Suggested cron: */30 * * * * /home/admin/cml-hive-assist/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send CmlHiveAssist notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., cml-hive-assist-alerts)
...
STATE_FILE="$HOME/.cml-hive-assist/auth-monitor-state"
...
send_notification() {
    ...
    # Send via CmlHiveAssist if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use cml-hive-assist
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via CmlHiveAssist to $NOTIFY_PHONE..."
            cml-hive-assist send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: CmlHiveAssist Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! CmlHiveAssist is down. Run: ssh l36 '~/cml-hive-assist/scripts/mobile-reauth.sh'" "urgent"
    exit 1
