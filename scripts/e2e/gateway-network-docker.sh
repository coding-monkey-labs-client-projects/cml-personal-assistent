...
IMAGE_NAME="cml-hive-assist-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="cml-hive-assist-net-e2e-$$"
GW_NAME="cml-hive-assist-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CML_HIVE_ASSIST_GATEWAY_TOKEN=$TOKEN" \
	  -e "CML_HIVE_ASSIST_SKIP_CHANNELS=1" \
	  -e "CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER=1" \
	  -e "CML_HIVE_ASSIST_SKIP_CRON=1" \
	  -e "CML_HIVE_ASSIST_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
