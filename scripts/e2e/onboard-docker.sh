...
IMAGE_NAME="cml-hive-assist-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/cml-hive-assist-bin:$PATH"
  mkdir -p /tmp/cml-hive-assist-bin
  cat > /tmp/cml-hive-assist-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/cml-hive-assist-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/cml-hive-assist-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/cml-hive-assist-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/cml-hive-assist-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.cml-hive-assist/workspace"
    config_path="$HOME/.cml-hive-assist/cml-hive-assist.json"
    sessions_dir="$HOME/.cml-hive-assist/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.cml-hive-assist/cml-hive-assist.json" <<'"'"'JSON'"'"'
{
