...
IMAGE_NAME="cml-hive-assist-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/cml-hive-assist-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/cml-hive-assist-bin/systemctl

  cat > /tmp/cml-hive-assist-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/cml-hive-assist-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/cml-hive-assist"
  npm_entry="/tmp/npm-prefix/lib/node_modules/cml-hive-assist/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/cml-hive-assist.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/cml-hive-assist-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/cml-hive-assist-gateway.service"
    if [ ! -f "$unit_path" ]; then
