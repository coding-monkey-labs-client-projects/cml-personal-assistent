#!/bin/bash
# Setup CmlHiveAssist Auth Management System
...
echo "=== CmlHiveAssist Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'cml-hive-assist-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  CmlHiveAssist message: Send warning via CmlHiveAssist itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/cml-hive-assist-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/cml-hive-assist-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/cml-hive-assist-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now cml-hive-assist-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status cml-hive-assist-auth-monitor.timer"
echo ""
echo "Quick commands:"
