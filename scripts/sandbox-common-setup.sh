...
BASE_IMAGE="${BASE_IMAGE:-cml-hive-assist-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-cml-hive-assist-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=cml-hive-assist.sandbox=1)
NOTE
