...
# Checks both Claude Code and CmlHiveAssist auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CML_HIVE_ASSIST_AUTH="$HOME/.cml-hive-assist/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    cml-hive-assist models status --json 2>/dev/null || true
}
...
check_cml-hive-assist_auth() {
    ...
    if [ ! -f "$CML_HIVE_ASSIST_AUTH" ]; then
        echo "MISSING"
        return 1
    fi
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$CML_HIVE_ASSIST_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    cml-hive-assist_status=$(check_cml-hive-assist_auth 2>/dev/null || true)

    claude_expires=0
    cml-hive-assist_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        cml-hive-assist_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        cml-hive-assist_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CML_HIVE_ASSIST_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$cml-hive-assist_status" \
        --arg be "$cml-hive-assist_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            cml-hive-assist: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$cml-hive-assist_status" == EXPIRED* ]] || [[ "$cml-hive-assist_status" == MISSING* ]]; then
        echo "CML_HIVE_ASSIST_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$cml-hive-assist_status" == EXPIRING* ]]; then
        echo "CML_HIVE_ASSIST_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "CmlHiveAssist Auth (~/.cml-hive-assist/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$CML_HIVE_ASSIST_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$CML_HIVE_ASSIST_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'cml-hive-assist doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active cml-hive-assist >/dev/null 2>&1; then
    echo -e "CmlHiveAssist service: ${GREEN}running${NC}"
else
    echo -e "CmlHiveAssist service: ${RED}NOT running${NC}"
fi
