...
show_usage() {
    cat << EOF
clawlog - CmlHiveAssist Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View CmlHiveAssist logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    CmlHiveAssist logs flow through the macOS unified log (subsystem: ai.cml-hive-assist).
