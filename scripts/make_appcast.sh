...
ZIP=${1:?"Usage: $0 CmlHiveAssist-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/cml-hive-assist/cml-hive-assist/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^CmlHiveAssist-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/cml-hive-assist/cml-hive-assist/releases/download/v${VERSION}/"}
