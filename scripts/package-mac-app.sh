...
# Build and bundle CmlHiveAssist into a minimal .app we can open.
# Outputs to dist/CmlHiveAssist.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/CmlHiveAssist.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="CmlHiveAssist"
BUNDLE_ID="${BUNDLE_ID:-ai.cml-hive-assist.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/cml-hive-assist/cml-hive-assist/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/CmlHiveAssist/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :CmlHiveAssistBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :CmlHiveAssistGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/CmlHiveAssist"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/CmlHiveAssist"
fi
chmod +x "$APP_ROOT/Contents/MacOS/CmlHiveAssist"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/CmlHiveAssist" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/CmlHiveAssist/Resources/CmlHiveAssist.icns" "$APP_ROOT/Contents/Resources/CmlHiveAssist.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/CmlHiveAssist/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying CmlHiveAssistKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/CmlHiveAssistKit_CmlHiveAssistKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/CmlHiveAssistKit_CmlHiveAssistKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/CmlHiveAssistKit_CmlHiveAssistKit.bundle"
else
  echo "WARN: CmlHiveAssistKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running CmlHiveAssist"
killall -q CmlHiveAssist 2>/dev/null || true
