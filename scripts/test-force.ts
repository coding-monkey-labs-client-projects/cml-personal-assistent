...
function runTests() {
  const isolatedLock =
    process.env.CML_HIVE_ASSIST_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `cml-hive-assist-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      CML_HIVE_ASSIST_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.CML_HIVE_ASSIST_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
