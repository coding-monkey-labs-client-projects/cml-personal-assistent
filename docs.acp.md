# CML Hive Assist ACP Bridge

This document describes how the CML Hive Assist ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`cml-hive-assist acp` exposes an ACP agent over stdio and forwards prompts to a running
CML Hive Assist Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
In Zed, open the Agent panel and select “CML Hive Assist ACP” to start a thread.

## Execution Model

- ACP client spawns `cml-hive-assist acp` and speaks ACP messages over stdio.
...
cml-hive-assist acp --reset-session
```
...
`cml-hive-assist acp` resolves the Gateway URL and auth from CLI flags or config:
