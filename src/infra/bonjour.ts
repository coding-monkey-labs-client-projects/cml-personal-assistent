...
import { ignoreCiaoCancellationRejection } from "./bonjour-ciao.ts";
import { formatBonjourError } from "./bonjour-errors.ts";
import { isTruthyEnvValue } from "./env.ts";
import { registerUnhandledRejectionHandler } from "./unhandled-rejections.ts";
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CML_HIVE_ASSIST_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "CmlHiveAssist";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(CmlHiveAssist\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.CML_HIVE_ASSIST_MDNS_HOSTNAME?.trim() ||
  ...
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
      .trim() || "cml-hive-assist";
  const instanceName =
    typeof opts.instanceName === "string" && opts.instanceName.trim()
      ? opts.instanceName.trim()
      : `${hostname} (CmlHiveAssist)`;
