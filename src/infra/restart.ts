...
export function triggerCmlHiveAssistRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CML_HIVE_ASSIST_SYSTEMD_UNIT,
        process.env.CML_HIVE_ASSIST_PROFILE,
      );
  ...
  const label =
    process.env.CML_HIVE_ASSIST_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CML_HIVE_ASSIST_PROFILE);
