import type { ChannelDirectoryEntryKind, ChannelId } from "../../channels/plugins/types.ts";
...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CmlHiveAssistConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: CmlHiveAssistConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CmlHiveAssistConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: CmlHiveAssistConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: CmlHiveAssistConfig): void {
