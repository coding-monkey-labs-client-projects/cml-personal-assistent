...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-update-"));
  });
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm cml-hive-assist doctor --non-interactive": { stdout: "" },
    });
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g cml-hive-assist@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
    ...
    expect(calls.some((call) => call === "npm i -g cml-hive-assist@latest")).toBe(true);
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "cml-hive-assist");
    const staleDir = path.join(nodeModules, ".cml-hive-assist-stale");
  ...
  it("updates global npm installs with tag override", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g cml-hive-assist@beta") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "cml-hive-assist", version: "2.0.0" }),
          "utf-8",
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g cml-hive-assist@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g cml-hive-assist@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "cml-hive-assist", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g cml-hive-assist@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a cml-hive-assist checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-cml-hive-assist-root");
