...
describe("ensureCmlHiveAssistCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling openclaw exists", async () => {
    ...
    try {
      ...
      const cliPath = path.join(appBinDir, "cml-hive-assist");
      ...
      delete process.env.CML_HIVE_ASSIST_PATH_BOOTSTRAPPED;
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.CML_HIVE_ASSIST_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "cml-hive-assist");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "cml-hive-assist");
