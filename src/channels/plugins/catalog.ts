...
import type { CmlHiveAssistPackageManifest } from "../../plugins/manifest.ts";
import type { PluginOrigin } from "../../plugins/types.ts";
import type { ChannelMeta } from "./types.ts";
import { MANIFEST_KEY } from "../../compat/legacy-names.ts";
import { discoverCmlHiveAssistPlugins } from "../../plugins/discovery.ts";
...
const ENV_CATALOG_PATHS = ["CML_HIVE_ASSIST_PLUGIN_CATALOG_PATHS", "CML_HIVE_ASSIST_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<CmlHiveAssistPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: CmlHiveAssistPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: CmlHiveAssistPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverCmlHiveAssistPlugins({ workspaceDir: options.workspaceDir });
