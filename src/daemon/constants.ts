...
export function resolveGatewayLaunchAgentLabel(profile?: string): string {
  ...
  return `ai.cml-hive-assist.${normalized}`;
}
...
export function resolveGatewayWindowsTaskName(profile?: string): string {
  ...
  return `CmlHiveAssist Gateway (${normalized})`;
}
...
export function formatGatewayServiceDescription(params?: {
  ...
  if (parts.length === 0) {
    return "CmlHiveAssist Gateway";
  }
  return `CmlHiveAssist Gateway (${parts.join(", ")})`;
}
...
export function formatNodeServiceDescription(params?: { version?: string }): string {
  const version = params?.version?.trim();
  if (!version) {
    return "CmlHiveAssist Node Host";
  }
  return `CmlHiveAssist Node Host (v${version})`;
}
