...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "cml-hive-assist" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["cml-hive-assist", "clawdbot", "moltbot"] as const;
...
function isCmlHiveAssistGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.cml-hive-assist.");
}

function isCmlHiveAssistGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
...
function isCmlHiveAssistGatewayTaskName(name: string): boolean {
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "cml-hive-assist" && isCmlHiveAssistGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "cml-hive-assist" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "cml-hive-assist" && isCmlHiveAssistGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isCmlHiveAssistGatewayTaskName(name)) {
        continue;
      }
