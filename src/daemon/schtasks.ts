...
import { formatGatewayServiceDescription, resolveGatewayWindowsTaskName } from "./constants.ts";
...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.CML_HIVE_ASSIST_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.CML_HIVE_ASSIST_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.CML_HIVE_ASSIST_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.CML_HIVE_ASSIST_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
