...
import { loadCmlHiveAssistPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.CML_HIVE_ASSIST_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "cml-hive-assist.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CML_HIVE_ASSIST_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadCmlHiveAssistPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CML_HIVE_ASSIST_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadCmlHiveAssistPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
