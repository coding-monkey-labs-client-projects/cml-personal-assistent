...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "cml-hive-assist-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "cml-hive-assist-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "cml-hive-assist-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.cml-hive-assist when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/cml-hive-assist")).toBe(
      path.resolve(os.homedir(), "cml-hive-assist"),
    );
  });
