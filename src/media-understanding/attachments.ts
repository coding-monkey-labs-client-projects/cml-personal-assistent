...
import { fetchRemoteMedia, MediaFetchError } from "../media/fetch.ts";
import { detectMime, getFileExtension, isAudioFileName, kindFromMime } from "../media/mime.ts";
import { MediaUnderstandingSkipError } from "./errors.ts";
import { fetchWithTimeout } from "./providers/shared.ts";
...
export class MediaAttachmentCache {
  ...
  async getPath(params: {
    ...
    const tmpPath = path.join(
      os.tmpdir(),
      `cml-hive-assist-media-${crypto.randomUUID()}${extension}`,
    );
