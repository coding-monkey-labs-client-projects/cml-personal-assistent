...
import { createCmlHiveAssistCodingTools } from "./pi-tools.js";

const defaultTools = createCmlHiveAssistCodingTools();

describe("createCmlHiveAssistCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createCmlHiveAssistCodingTools();
    ...
    try {
      ...
      const contents = "Hello from cml-hive-assist read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "cml-hive-assist-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "cml-hive-assist-workspace"),
    ...
    const tools = createCmlHiveAssistCodingTools({ sandbox });
