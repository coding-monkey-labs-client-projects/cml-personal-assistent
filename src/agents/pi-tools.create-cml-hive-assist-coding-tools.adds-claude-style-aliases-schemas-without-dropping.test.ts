...
import { createCmlHiveAssistTools } from "./cml-hive-assist-tools.js";
import { __testing, createCmlHiveAssistCodingTools } from "./pi-tools.js";
...
describe("createCmlHiveAssistCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCmlHiveAssistCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "cml-hive-assist-outside.txt");
