...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.CML_HIVE_ASSIST_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: cml-hive-assist --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/cml-hive-assist" } }\n}\n';
