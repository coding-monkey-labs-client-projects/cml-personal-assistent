...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureCmlHiveAssistModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureCmlHiveAssistModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureCmlHiveAssistModelsJson } = await import("./models-config.js");
      const { resolveCmlHiveAssistAgentDir } = await import("./agent-paths.js");

      await ensureCmlHiveAssistModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveCmlHiveAssistAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureCmlHiveAssistModelsJson({});
