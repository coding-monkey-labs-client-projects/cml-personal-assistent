...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.CML_HIVE_ASSIST_SKIP_BROWSER_CONTROL_SERVER,
    token: process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN,
    password: process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD,
  };
  ...
  afterAll(async () => {
    ...
    process.env.CML_HIVE_ASSIST_STATE_DIR = prev.stateDir;
    process.env.CML_HIVE_ASSIST_CONFIG_PATH = prev.configPath;
    process.env.CML_HIVE_ASSIST_SKIP_CHANNELS = prev.skipChannels;
    process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.CML_HIVE_ASSIST_SKIP_CRON = prev.skipCron;
    process.env.CML_HIVE_ASSIST_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.CML_HIVE_ASSIST_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = prev.token;
    process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "cml-hive-assist");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CML_HIVE_ASSIST_STATE_DIR = stateDir;
    process.env.CML_HIVE_ASSIST_CONFIG_PATH = path.join(stateDir, "cml-hive-assist.json");
