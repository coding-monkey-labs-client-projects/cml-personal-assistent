...
beforeEach(() => {
  ...
  resolveCmlHiveAssistPackageRoot.mockReset().mockResolvedValue(null);
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CML_HIVE_ASSIST_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CML_HIVE_ASSIST_UPDATE_IN_PROGRESS;
  } else {
    process.env.CML_HIVE_ASSIST_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveCmlHiveAssistPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadCmlHiveAssistPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "cml-hive-assist-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CML_HIVE_ASSIST_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CML_HIVE_ASSIST_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/cml-hive-assist.json",
      exists: true,
      raw: "{}",
    ...
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
