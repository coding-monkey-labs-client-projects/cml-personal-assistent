...
const requireAgentDir = () => {
  const agentDir = process.env.CML_HIVE_ASSIST_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CML_HIVE_ASSIST_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("applyAuthChoice", () => {
  ...
  it("prompts and writes MiniMax API key when selecting minimax-api", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-auth-"));
