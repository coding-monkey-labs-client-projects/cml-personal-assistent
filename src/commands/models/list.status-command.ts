...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("cml-hive-assist models auth setup-token")}\` or \`${formatCliCommand("cml-hive-assist configure")}\`.`
          : `Run \`${formatCliCommand("cml-hive-assist configure")}\` or set an API key env var.`;
