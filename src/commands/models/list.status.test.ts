...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCmlHiveAssistAgentDir: vi.fn().mockReturnValue("/tmp/cml-hive-assist-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/cml-hive-assist-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveCmlHiveAssistAgentDir: mocks.resolveCmlHiveAssistAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCmlHiveAssistAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/cml-hive-assist-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/cml-hive-assist-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/cml-hive-assist-agent-custom");
