import type { CmlHiveAssistConfig, ConfigFileSnapshot } from "../../config/types.ts";
...
export function resolveTargets(cfg: CmlHiveAssistConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: CmlHiveAssistConfig) {
