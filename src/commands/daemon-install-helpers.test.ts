...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ CML_HIVE_ASSIST_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ CML_HIVE_ASSIST_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.CML_HIVE_ASSIST_PORT).toBe("3000");
