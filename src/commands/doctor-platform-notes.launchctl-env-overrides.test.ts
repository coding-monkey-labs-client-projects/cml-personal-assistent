...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CML_HIVE_ASSIST_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CML_HIVE_ASSIST_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CML_HIVE_ASSIST_GATEWAY_TOKEN");
    expect(message).not.toContain("CML_HIVE_ASSIST_GATEWAY_PASSWORD");
  });
