import type { BrowserConfig, BrowserProfileConfig, CmlHiveAssistConfig } from "../config/config.ts";
import { resolveGatewayPort } from "../config/paths.ts";
import {
  deriveDefaultBrowserCdpPortRange,
  deriveDefaultBrowserControlPort,
  DEFAULT_BROWSER_CONTROL_PORT,
} from "../config/port-defaults.ts";
import {
  DEFAULT_CML_HIVE_ASSIST_BROWSER_COLOR,
  DEFAULT_CML_HIVE_ASSIST_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_CML_HIVE_ASSIST_BROWSER_PROFILE_NAME,
} from "./constants.ts";
import { CDP_PORT_RANGE_START, getUsedPorts } from "./profiles.ts";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_CML_HIVE_ASSIST_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "cml-hive-assist" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CML_HIVE_ASSIST_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CML_HIVE_ASSIST_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an CmlHiveAssist browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for cml-hive-assist/cml-hive-assist CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: CmlHiveAssistConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CML_HIVE_ASSIST_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "cml-hive-assist";
