...
describe("browser config", () => {
  ...
  it("derives default ports from CML_HIVE_ASSIST_GATEWAY_PORT when unset", () => {
    const prev = process.env.CML_HIVE_ASSIST_GATEWAY_PORT;
    process.env.CML_HIVE_ASSIST_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.CML_HIVE_ASSIST_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "cml-hive-assist");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    ...
    expect(resolved.defaultProfile).toBe("cml-hive-assist");
  });
});
