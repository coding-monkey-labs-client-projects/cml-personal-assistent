...
/**
 * Nix mode detection: When CML_HIVE_ASSIST_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.CML_HIVE_ASSIST_NIX_MODE === "1";
}
...
export function resolveStateDir(
  ...
  const override = env.CML_HIVE_ASSIST_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.CML_HIVE_ASSIST_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
...
export function resolveConfigPath(
  ...
  const override = env.CML_HIVE_ASSIST_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const stateOverride = env.CML_HIVE_ASSIST_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const explicit = env.CML_HIVE_ASSIST_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
 ...
 *
 * Precedence:
 * - `CML_HIVE_ASSIST_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.CML_HIVE_ASSIST_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.CML_HIVE_ASSIST_GATEWAY_PORT?.trim() || env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
