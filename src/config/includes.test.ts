...
const ETC_CML_HIVE_ASSIST_DIR = path.join(ROOT_DIR, "etc", "cml-hive-assist");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "cml-hive-assist.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcCmlHiveAssistPath(...parts: string[]) {
  return path.join(ETC_CML_HIVE_ASSIST_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcCmlHiveAssistPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "cml-hive-assist.json"))).toEqual({
      shared: true,
    });
  });
});
