export type CmlHiveAssistVersion = {
  major: number;
  minor: number;
  patch: number;
  revision: number;
...
export function compareCmlHiveAssistVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseCmlHiveAssistVersion(a);
  const parsedB = parseCmlHiveAssistVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
