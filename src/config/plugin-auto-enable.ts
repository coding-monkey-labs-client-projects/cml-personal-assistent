...
import { hasAnyWhatsAppAuth } from "../web/accounts.ts";
...
function isTelegramConfigured(cfg: CmlHiveAssistConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: CmlHiveAssistConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: CmlHiveAssistConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: CmlHiveAssistConfig): boolean {
...
function isIMessageConfigured(cfg: CmlHiveAssistConfig): boolean {
...
function isWhatsAppConfigured(cfg: CmlHiveAssistConfig): boolean {
...
function isGenericChannelConfigured(cfg: CmlHiveAssistConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: CmlHiveAssistConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: CmlHiveAssistConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: CmlHiveAssistConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: CmlHiveAssistConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: CmlHiveAssistConfig, pluginId: string): CmlHiveAssistConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
