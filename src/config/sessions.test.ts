...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-cml-hive-assist",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-cml-hive-assist#general");
  });
  ...
  it("derives session transcripts dir from CML_HIVE_ASSIST_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CML_HIVE_ASSIST_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CML_HIVE_ASSIST_STATE_DIR = "/custom/state";
    try {
