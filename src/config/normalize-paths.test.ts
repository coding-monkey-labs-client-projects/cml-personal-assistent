  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.cml-hive-assist/logs/cml-hive-assist.log" },
        hooks: {
          path: "~/.cml-hive-assist/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.cml-hive-assist/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(
        path.join(home, ".cml-hive-assist", "logs", "cml-hive-assist.log"),
      );
      expect(cfg.hooks?.path).toBe(path.join(home, ".cml-hive-assist", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".cml-hive-assist", "telegram.token"),
      );
