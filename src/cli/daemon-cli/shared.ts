...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("cml-hive-assist gateway install", env)}`,
    );
    if (fileLog) {
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("cml-hive-assist gateway install", env),
    formatCliCommand("cml-hive-assist gateway", env),
  ];
