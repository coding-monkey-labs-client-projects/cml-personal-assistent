...
import { canvasSnapshotTempPath, parseCanvasSnapshotPayload } from "../nodes-canvas.ts";
import { parseTimeoutMs } from "../nodes-run.ts";
import { buildA2UITextJsonl, validateA2UIJsonl } from "./a2ui-jsonl.ts";
    ...
    a2ui
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("canvas a2ui push", async () => {
          ...
          if (version === "v0.9") {
            throw new Error(
              "Detected A2UI v0.9 JSONL (createSurface). CmlHiveAssist currently supports v0.8 only.",
            );
          }
