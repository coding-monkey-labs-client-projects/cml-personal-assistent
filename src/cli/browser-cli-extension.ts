...
import { movePathToTrash } from "../browser/trash.ts";
...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall CmlHiveAssist and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “CmlHiveAssist Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.cml-hive-assist.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
