...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CML_HIVE_ASSIST_SKIP_CHANNELS;
    prevSkipGmail = process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER;
    process.env.CML_HIVE_ASSIST_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CML_HIVE_ASSIST_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
