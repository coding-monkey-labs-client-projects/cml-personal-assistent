...
beforeAll(async () => {
  previousToken = process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server health/presence", () => {
  ...
  test("presence includes client fingerprint", async () => {
    const identityPath = path.join(os.tmpdir(), `cml-hive-assist-device-${randomUUID()}.json`);
