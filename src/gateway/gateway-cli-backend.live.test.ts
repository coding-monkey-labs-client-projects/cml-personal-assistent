...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("CML_HIVE_ASSIST_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.CML_HIVE_ASSIST_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `CML_HIVE_ASSIST_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const baseCliArgs =
      parseJsonStringArray(
        "CML_HIVE_ASSIST_LIVE_CLI_BACKEND_ARGS",
    ...
    const cliClearEnv =
      parseJsonStringArray(
        "CML_HIVE_ASSIST_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.CML_HIVE_ASSIST_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.CML_HIVE_ASSIST_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.CML_HIVE_ASSIST_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "CML_HIVE_ASSIST_LIVE_CLI_BACKEND_IMAGE_MODE requires CML_HIVE_ASSIST_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }
    ...
    try {
      ...
      if (previous.skipChannels === undefined) {
        delete process.env.CML_HIVE_ASSIST_SKIP_CHANNELS;
      } else {
      ...
      if (previous.skipCron === undefined) {
        delete process.env.CML_HIVE_ASSIST_SKIP_CRON;
      } else {
