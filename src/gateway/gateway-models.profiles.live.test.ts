...
import type { CmlHiveAssistConfig, ModelProviderConfig } from "../config/types.js";
import { resolveCmlHiveAssistAgentDir } from "../agents/agent-paths.js";
...
import { ensureCmlHiveAssistModelsJson } from "../agents/models-config.js";
...
function sanitizeAuthConfig(params: {
  cfg: CmlHiveAssistConfig;
  agentDir: string;
}): CmlHiveAssistConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<CmlHiveAssistConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.CML_HIVE_ASSIST_CONFIG_PATH,
    token: process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN,
    skipChannels: process.env.CML_HIVE_ASSIST_SKIP_CHANNELS,
    skipGmail: process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER,
    skipCron: process.env.CML_HIVE_ASSIST_SKIP_CRON,
    skipCanvas: process.env.CML_HIVE_ASSIST_SKIP_CANVAS_HOST,
    agentDir: process.env.CML_HIVE_ASSIST_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CML_HIVE_ASSIST_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveCmlHiveAssistAgentDir();
  ...
  process.env.CML_HIVE_ASSIST_AGENT_DIR = tempAgentDir;
  ...
  const sanitizedCfg: CmlHiveAssistConfig = {
    ...params.cfg,
  ...
  await ensureCmlHiveAssistModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "CmlHiveAssist live tool probe (local, safe): " +
    ...
    process.env.CML_HIVE_ASSIST_CONFIG_PATH = previous.configPath;
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = previous.token;
    process.env.CML_HIVE_ASSIST_SKIP_CHANNELS = previous.skipChannels;
    process.env.CML_HIVE_ASSIST_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CML_HIVE_ASSIST_SKIP_CRON = previous.skipCron;
    process.env.CML_HIVE_ASSIST_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CML_HIVE_ASSIST_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CML_HIVE_ASSIST_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.CML_HIVE_ASSIST_LIVE_GATEWAY_MODELS?.trim();
