...
import { CANVAS_HOST_PATH } from "../canvas-host/a2ui.ts";
import { type CanvasHostHandler, createCanvasHostHandler } from "../canvas-host/server.ts";
import { resolveGatewayListenHosts } from "./net.ts";
import { createGatewayBroadcaster } from "./server-broadcast.ts";
import { type ChatRunEntry, createChatRunState } from "./server-chat.ts";
import { MAX_PAYLOAD_BYTES } from "./server-constants.ts";
import { attachGatewayUpgradeHandler, createGatewayHttpServer } from "./server-http.ts";
import { createGatewayHooksRequestHandler } from "./server/hooks.ts";
import { listenGatewayHttpServer } from "./server/http-listen.ts";
import { createGatewayPluginRequestHandler } from "./server/plugins-http.ts";

export async function createGatewayRuntimeState(params: {
  cfg: import("../config/config.ts").CmlHiveAssistConfig;
  bindHost: string;
  port: number;
