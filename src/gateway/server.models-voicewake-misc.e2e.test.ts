...
describe("gateway server models + voicewake", () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(initial.payload?.triggers).toEqual(["cml-hive-assist", "claude", "computer"]);
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
      triggers: ["cml-hive-assist", "computer"],
    });
    expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.CML_HIVE_ASSIST_CANVAS_HOST_PORT;
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.CML_HIVE_ASSIST_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.CML_HIVE_ASSIST_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.CML_HIVE_ASSIST_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing CML_HIVE_ASSIST_CONFIG_PATH");
    }
