...
import { type HookMappingResolved, resolveHookMappings } from "./hooks-mapping.ts";
...
export function resolveHooksConfig(cfg: CmlHiveAssistConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-cml-hive-assist-token"] === "string"
      ? req.headers["x-cml-hive-assist-token"].trim()
      : "";
  if (headerToken) {
