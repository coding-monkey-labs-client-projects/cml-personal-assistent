...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.CML_HIVE_ASSIST_GATEWAY_PORT = String(gatewayPort);
  process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createCmlHiveAssistTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
