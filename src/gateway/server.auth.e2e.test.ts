...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CML_HIVE_ASSIST_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CML_HIVE_ASSIST_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CML_HIVE_ASSIST_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CML_HIVE_ASSIST_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN;
