...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevBundledDir = process.env.CML_HIVE_ASSIST_BUNDLED_HOOKS_DIR;
    process.env.CML_HIVE_ASSIST_STATE_DIR = path.join(baseDir, "state");
    process.env.CML_HIVE_ASSIST_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.CML_HIVE_ASSIST_BUNDLED_HOOKS_DIR;
    } else {
      process.env.CML_HIVE_ASSIST_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
