...
homepage: https://docs.cml-hive-assist.ai/hooks#command-logger
metadata:
  {
    "cml-hive-assist":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.cml-hive-assist/logs/commands.log`
...
`~/.cml-hive-assist/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.cml-hive-assist/logs/commands.log ~/.cml-hive-assist/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/cml-hive-assist`:
   ```
   /home/username/.cml-hive-assist/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
