...
This directory contains hooks that ship with CmlHiveAssist. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.cml-hive-assist.ai/hooks/soul-evil
...
Hooks can be configured in `~/.cml-hive-assist/cml-hive-assist.json`:
...
2. Restart gateway: `pkill -9 -f 'cml-hive-assist.*gateway' && pnpm cml-hive-assist gateway`
3. Enable the hook: `cml-hive-assist hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.cml-hive-assist.ai/hooks
