...
describe("security audit", () => {
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: CmlHiveAssistConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.CML_HIVE_ASSIST_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: CmlHiveAssistConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: CmlHiveAssistConfig = {
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: CmlHiveAssistConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "cml-hive-assist.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD;
      } else {
        process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.CML_HIVE_ASSIST_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: CmlHiveAssistConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.CML_HIVE_ASSIST_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: CmlHiveAssistConfig = {
        gateway: {
          mode: "remote",
