...
import { resolveBrowserConfig, resolveProfile } from "../browser/config.ts";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your CmlHiveAssist state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your CmlHiveAssist state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectBrowserControlFindings(cfg: CmlHiveAssistConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectLoggingFindings(cfg: CmlHiveAssistConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CmlHiveAssistConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
