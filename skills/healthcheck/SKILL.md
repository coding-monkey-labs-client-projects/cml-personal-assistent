---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for CmlHiveAssist deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, CmlHiveAssist cron scheduling for periodic checks, or version status checks on a machine running CmlHiveAssist (laptop, workstation, Pi, VPS).
---

# CmlHiveAssist Host Hardening

## Overview

Assess and harden the host running CmlHiveAssist, then align it to a user-defined risk tolerance without breaking access. Use CmlHiveAssist security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CmlHiveAssist changes the host firewall, SSH, or OS updates; it does not.
...
5. CmlHiveAssist gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CmlHiveAssist can access sensitive data.
...
### 2) Run CmlHiveAssist security audits (read-only)

As part of the default read-only checks, run `cml-hive-assist security audit --deep`. Only offer alternatives if the user requests them:

1. `cml-hive-assist security audit` (faster, non-probing)
2. `cml-hive-assist security audit --json` (structured output)

Offer to apply CmlHiveAssist safe defaults (numbered):

1. `cml-hive-assist security audit --fix`

Be explicit that `--fix` only tightens CmlHiveAssist defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CmlHiveAssist version/update status (read-only)

As part of the default read-only checks, run `cml-hive-assist update status`.
...
- Credential hygiene notes (location of CmlHiveAssist creds, prefer disk encryption)
...
- CmlHiveAssist security audit (re-run)
...
After CmlHiveAssist install or first hardening pass, run at least one baseline audit and version check:

- `cml-hive-assist security audit`
- `cml-hive-assist security audit --deep`
- `cml-hive-assist update status`

Ongoing monitoring is recommended. Use the CmlHiveAssist cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `cml-hive-assist cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `cml-hive-assist update status`
...
Before creating, `cml-hive-assist cron list` and match on exact `name`. If found, `cml-hive-assist cron edit <id> ...`.
If not found, `cml-hive-assist cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `cml-hive-assist update status` (preferred for source checkouts and channels)
2. `npm view cml-hive-assist version` (published npm version)

## CmlHiveAssist command accuracy

Use only supported commands and flags:

- `cml-hive-assist security audit [--deep] [--fix] [--json]`
- `cml-hive-assist status` / `cml-hive-assist status --deep`
- `cml-hive-assist health --json`
- `cml-hive-assist update status`
- `cml-hive-assist cron add|list|runs|run`

Do not invent CLI flags or imply CmlHiveAssist enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by CmlHiveAssist compaction:
