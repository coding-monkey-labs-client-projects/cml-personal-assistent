...
export async function handleWhatsAppStart(host: CmlHiveAssistApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CmlHiveAssistApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CmlHiveAssistApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CmlHiveAssistApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CmlHiveAssistApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CmlHiveAssistApp): string {
...
export function handleNostrProfileCancel(host: CmlHiveAssistApp) {
...
export function handleNostrProfileToggleAdvanced(host: CmlHiveAssistApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: CmlHiveAssistApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CmlHiveAssistApp) {
  const state = host.nostrProfileFormState;
