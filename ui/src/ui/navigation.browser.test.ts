...
const originalConnect = CmlHiveAssistApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("cml-hive-assist-app") as CmlHiveAssistApp;
...
beforeEach(() => {
  CmlHiveAssistApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__CML_HIVE_ASSIST_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  CmlHiveAssistApp.prototype.connect = originalConnect;
  window.__CML_HIVE_ASSIST_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/cml-hive-assist/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/cml-hive-assist");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/cml-hive-assist/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CML_HIVE_ASSIST_CONTROL_UI_BASE_PATH__ = "/cml-hive-assist";
    const app = mountApp("/cml-hive-assist/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/cml-hive-assist");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/cml-hive-assist/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "cml-hive-assist.control.settings.v1",
