...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: CmlHiveAssistProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CmlHiveAssistProtocol.AnyCodable] = [
            "presence": .init([CmlHiveAssistProtocol.AnyCodable(entry)]),
        ]
        let payload = CmlHiveAssistProtocol.AnyCodable(payloadMap)
