...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so CmlHiveAssist can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = CmlHiveAssistLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = CmlHiveAssistLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(CmlHiveAssistLocationMode.off.rawValue)
                Text("While Using").tag(CmlHiveAssistLocationMode.whileUsing.rawValue)
                Text("Always").tag(CmlHiveAssistLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = CmlHiveAssistLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: CmlHiveAssistLocationMode {
        CmlHiveAssistLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: CmlHiveAssistLocationMode) async -> Bool {
