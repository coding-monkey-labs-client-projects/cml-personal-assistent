...
enum DebugActions {
    private static let verboseDefaultsKey = "cml-hive-assist.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "cml-hive-assist.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = CmlHiveAssistPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "CmlHiveAssist", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CmlHiveAssistPaths.configURL
        guard
