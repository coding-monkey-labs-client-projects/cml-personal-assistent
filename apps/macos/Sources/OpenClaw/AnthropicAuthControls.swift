...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: CmlHiveAssistOAuthStore.AnthropicOAuthStatus = CmlHiveAssistOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: CmlHiveAssistOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = CmlHiveAssistOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = CmlHiveAssistOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try CmlHiveAssistOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. CmlHiveAssist can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: CmlHiveAssistOAuthStore.AnthropicOAuthStatus,
