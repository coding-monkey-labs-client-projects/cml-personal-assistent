...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.cml-hive-assist", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: CmlHiveAssistPeekabooBridgeServices?
    private static var cml-hive-assistSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("CmlHiveAssist", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = CmlHiveAssistPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.cml-hive-assistSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.cml-hive-assistSocketPath, privacy: .public)")
    }
...
@MainActor
private final class CmlHiveAssistPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.cml-hive-assist.peekaboo")
