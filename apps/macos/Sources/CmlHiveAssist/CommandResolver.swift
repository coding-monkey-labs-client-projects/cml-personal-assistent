...
enum CommandResolver {
    private static let projectRootDefaultsKey = "cml-hive-assist.gatewayProjectRootPath"
    private static let helperName = "cml-hive-assist"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let cml-hive-assistEntry = root.appendingPathComponent("cml-hive-assist.mjs").path
        if FileManager().isReadableFile(atPath: cml-hive-assistEntry) { return cml-hive-assistEntry }
        let binEntry = root.appendingPathComponent("bin/cml-hive-assist.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__CML_HIVE_ASSIST_ERR__' >&2
        \(message)
        __CML_HIVE_ASSIST_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/cml-hive-assist")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let cml-hive-assistPaths = self.cml-hive-assistManagedPaths(home: home)
        if !cml-hive-assistPaths.isEmpty {
            extras.insert(contentsOf: cml-hive-assistPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + cml-hive-assistPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func cml-hive-assistManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".cml-hive-assist"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func cml-hive-assistExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCmlHiveAssistExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("cml-hive-assist.mjs").path,
            root.appendingPathComponent("bin/cml-hive-assist.js").path,
        ]
    ...
    static func hasAnyCmlHiveAssistInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.cml-hive-assistExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func cml-hive-assistNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let cml-hive-assistPath = self.projectCmlHiveAssistExecutable(projectRoot: root) {
                return [cml-hive-assistPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "cml-hive-assist", subcommand] + extraArgs
            }
            if let cml-hive-assistPath = self.cml-hive-assistExecutable(searchPaths: searchPaths) {
                return [cml-hive-assistPath, subcommand] + extraArgs
            }

            let missingEntry = """
            cml-hive-assist entrypoint missing (looked for dist/index.js or cml-hive-assist.mjs); run pnpm build.
            """
    ...
    static func cml-hive-assistCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.cml-hive-assistNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real cml-hive-assist CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/cml-hive-assist"
        ...
        \(cliSection)
        \(projectSection)
        if command -v cml-hive-assist >/dev/null 2>&1; then
          CLI="$(command -v cml-hive-assist)"
          cml-hive-assist \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/cml-hive-assist.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/cml-hive-assist.mjs"
            node "$PRJ/cml-hive-assist.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/cml-hive-assist.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/cml-hive-assist.js"
            node "$PRJ/bin/cml-hive-assist.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent cml-hive-assist"
          pnpm --silent cml-hive-assist \(quotedArgs);
        else
          echo "cml-hive-assist CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? CmlHiveAssistConfigFile.loadDict()
