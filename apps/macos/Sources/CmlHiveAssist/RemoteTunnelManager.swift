...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.cml-hive-assist", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous CmlHiveAssist run already has an SSH listener on the expected port (common after restarts),
