...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.cml-hive-assist", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CmlHiveAssistSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CmlHiveAssistSessionPreviewEntry,
    ...
    private static func cache(payload: CmlHiveAssistSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CmlHiveAssistChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CmlHiveAssistKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CmlHiveAssistKit.AnyCodable]) -> [CmlHiveAssistChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CmlHiveAssistChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CmlHiveAssistChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CmlHiveAssistChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CmlHiveAssistChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CmlHiveAssistChatMessage) -> String? {
