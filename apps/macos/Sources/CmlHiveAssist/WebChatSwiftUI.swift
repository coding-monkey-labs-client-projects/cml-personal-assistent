...
private let webChatSwiftLogger = Logger(subsystem: "ai.cml-hive-assist", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: CmlHiveAssistChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CmlHiveAssistChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(CmlHiveAssistChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CmlHiveAssistChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CmlHiveAssistGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    CmlHiveAssistChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    CmlHiveAssistAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CmlHiveAssistChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CmlHiveAssistChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CmlHiveAssistChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: CmlHiveAssistChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "CmlHiveAssist Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CmlHiveAssistChatView>) -> NSViewController
    {
