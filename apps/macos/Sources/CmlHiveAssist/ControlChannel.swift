...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CmlHiveAssistProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.cml-hive-assist", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CmlHiveAssistKit.AnyCodable]()) {
                $0[$1.key] = CmlHiveAssistKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CmlHiveAssistProtocol.AnyCodable?) -> [String: CmlHiveAssistProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CmlHiveAssistProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CmlHiveAssistKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CmlHiveAssistProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("cml-hive-assist.control.heartbeat")
    static let controlAgentEvent = Notification.Name("cml-hive-assist.control.agent")
}
