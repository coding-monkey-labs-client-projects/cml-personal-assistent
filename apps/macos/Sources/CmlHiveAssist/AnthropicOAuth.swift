...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (CmlHiveAssist token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: CmlHiveAssistOAuthStore.AnthropicOAuthStatus = CmlHiveAssistOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.cml-hive-assist", category: "anthropic-oauth")
...
enum CmlHiveAssistOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let cml-hive-assistOAuthDirEnv = "CML_HIVE_ASSIST_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "CmlHiveAssist OAuth token file not found"
            case .unreadableFile: "CmlHiveAssist OAuth token file not readable"
            case .invalidJSON: "CmlHiveAssist OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in CmlHiveAssist OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "CmlHiveAssist OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.cml-hive-assistOAuthDirEnv]?
