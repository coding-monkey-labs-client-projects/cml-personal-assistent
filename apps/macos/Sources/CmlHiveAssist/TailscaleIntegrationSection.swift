...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.cml-hive-assist/cml-hive-assist.json. Prefer CML_HIVE_ASSIST_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.cml-hive-assist/cml-hive-assist.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.cml-hive-assist/cml-hive-assist.json. Restart the gateway to apply."
        }
