...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.cml-hive-assist", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["CML_HIVE_ASSIST_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = CmlHiveAssistConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.cml-hive-assistExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "cml-hive-assist CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["CML_HIVE_ASSIST_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "cml-hive-assist@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "cml-hive-assist@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "cml-hive-assist@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "cml-hive-assist@\(target)"])
            }

        statusHandler("Installing cml-hive-assist@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed cml-hive-assist@\(target)")
        } else {
