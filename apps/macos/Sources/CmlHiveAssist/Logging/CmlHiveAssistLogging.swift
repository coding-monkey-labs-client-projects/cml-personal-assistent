...
enum CmlHiveAssistLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = CmlHiveAssistOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CmlHiveAssistFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("ai.cml-hive-assist", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CmlHiveAssistLogging.bootstrapIfNeeded()
        let label = CmlHiveAssistLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CmlHiveAssistOSLogHandler: LogHandler {
...
struct CmlHiveAssistFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CmlHiveAssistLogging.parseLabel(self.label)
