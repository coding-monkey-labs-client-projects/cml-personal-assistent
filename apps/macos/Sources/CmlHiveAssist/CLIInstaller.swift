...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("cml-hive-assist").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing cml-hive-assist CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed cml-hive-assist \($0)." } ?? "Installed cml-hive-assist."
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".cml-hive-assist")
            .path
    }
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://cml-hive-assist.bot/install-cli.sh | \
