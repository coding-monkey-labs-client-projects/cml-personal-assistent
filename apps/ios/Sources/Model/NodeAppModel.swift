...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = CmlHiveAssistCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = CmlHiveAssistCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = CmlHiveAssistCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = CmlHiveAssistCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = CmlHiveAssistCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__cml-hive-assist__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: CmlHiveAssistLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for cml-hive-assist:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case CmlHiveAssistLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case CmlHiveAssistCanvasCommand.present.rawValue,
            ...
            case CmlHiveAssistCanvasA2UICommand.reset.rawValue,
            ...
            case CmlHiveAssistCameraCommand.list.rawValue,
                 CmlHiveAssistCameraCommand.snap.rawValue,
                 CmlHiveAssistCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case CmlHiveAssistScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: CmlHiveAssistNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CmlHiveAssistNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(CmlHiveAssistLocationGetParams.self, from: req.paramsJSON)) ??
            CmlHiveAssistLocationGetParams()
        ...
        let payload = CmlHiveAssistLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CmlHiveAssistCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(CmlHiveAssistCanvasPresentParams.self, from: req.paramsJSON)) ??
                CmlHiveAssistCanvasPresentParams()
        ...
        case CmlHiveAssistCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CmlHiveAssistCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(CmlHiveAssistCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CmlHiveAssistCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(CmlHiveAssistCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CmlHiveAssistCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(CmlHiveAssistCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CmlHiveAssistCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing cml-hive-assistA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CmlHiveAssistCanvasA2UICommand.push.rawValue, CmlHiveAssistCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == CmlHiveAssistCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(CmlHiveAssistCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try CmlHiveAssistCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(CmlHiveAssistCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try CmlHiveAssistCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.cml-hive-assistA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CmlHiveAssistCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CmlHiveAssistCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(CmlHiveAssistCameraSnapParams.self, from: req.paramsJSON)) ??
                CmlHiveAssistCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CmlHiveAssistCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(CmlHiveAssistCameraClipParams.self, from: req.paramsJSON)) ??
                CmlHiveAssistCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(CmlHiveAssistScreenRecordParams.self, from: req.paramsJSON)) ??
            CmlHiveAssistScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> CmlHiveAssistLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return CmlHiveAssistLocationMode(rawValue: raw) ?? .off
    }
