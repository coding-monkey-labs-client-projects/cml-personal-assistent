...
struct IOSGatewayChatTransport: CmlHiveAssistChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CmlHiveAssistChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(CmlHiveAssistChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CmlHiveAssistChatHistoryPayload {
        ...
        return try JSONDecoder().decode(CmlHiveAssistChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [CmlHiveAssistChatAttachmentPayload]) async throws -> CmlHiveAssistChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [CmlHiveAssistChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(CmlHiveAssistChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(CmlHiveAssistGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CmlHiveAssistGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CmlHiveAssistChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CmlHiveAssistAgentEventPayload.self)
                        {
