...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func handleRecognitionCallbackDispatchesCommand() async {
        let manager = VoiceWakeManager()
        manager.triggerWords = ["cml-hive-assist"]
        manager.isEnabled = true
        ...
        let transcript = "cml-hive-assist hello"
        let triggerRange = transcript.range(of: "cml-hive-assist")!
        let helloRange = transcript.range(of: "hello")!
        let segments = [
            WakeWordSegment(text: "cml-hive-assist", start: 0.0, duration: 0.2, range: triggerRange),
