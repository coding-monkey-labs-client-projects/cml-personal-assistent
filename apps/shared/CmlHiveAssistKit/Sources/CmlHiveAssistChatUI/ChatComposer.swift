...
@MainActor
struct CmlHiveAssistChatComposer: View {
    @Bindable var viewModel: CmlHiveAssistChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(CmlHiveAssistChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(CmlHiveAssistChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \CmlHiveAssistPendingAttachment.id)
                { (att: CmlHiveAssistPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            CmlHiveAssistPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(CmlHiveAssistChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CmlHiveAssistChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(CmlHiveAssistChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(CmlHiveAssistChatTheme.subtleCard)
        .clipShape(Capsule())
    }
