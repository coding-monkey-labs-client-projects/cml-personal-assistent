...
struct ChatMessageBubble: View {
    let message: CmlHiveAssistChatMessage
    let style: CmlHiveAssistChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CmlHiveAssistChatTheme.userText : CmlHiveAssistChatTheme.assistantText
    ...
    private var inlineAttachments: [CmlHiveAssistChatMessageContent] {
    ...
    private var toolCalls: [CmlHiveAssistChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CmlHiveAssistChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CmlHiveAssistChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CmlHiveAssistChatTheme.onboardingAssistantBubble
        }
        return CmlHiveAssistChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CmlHiveAssistChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CmlHiveAssistChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CmlHiveAssistChatTheme.userText : CmlHiveAssistChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CmlHiveAssistChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CmlHiveAssistChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            if self.style == .standard {
                Text("CmlHiveAssist is thinking…")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                Spacer()
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CmlHiveAssistChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CmlHiveAssistChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CmlHiveAssistChatTheme.assistantText)
            }
        }
    }
}
