...
private let chatUILogger = Logger(subsystem: "ai.cml-hive-assist", category: "CmlHiveAssistChatUI")

@MainActor
@Observable
public final class CmlHiveAssistChatViewModel {
    public private(set) var messages: [CmlHiveAssistChatMessage] = []
    ...
    public var attachments: [CmlHiveAssistPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CmlHiveAssistChatPendingToolCall] = []
    public private(set) var sessions: [CmlHiveAssistChatSessionEntry] = []
    private let transport: any CmlHiveAssistChatTransport
    ...
    private var pendingToolCallsById: [String: CmlHiveAssistChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any CmlHiveAssistChatTransport) {
    ...
    public var sessionChoices: [CmlHiveAssistChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [CmlHiveAssistChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [CmlHiveAssistChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CmlHiveAssistPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CmlHiveAssistChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CmlHiveAssistChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [CmlHiveAssistChatMessage]) -> [CmlHiveAssistChatMessage] {
        var result: [CmlHiveAssistChatMessage] = []
    ...
    private static func dedupeKey(for message: CmlHiveAssistChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CmlHiveAssistChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> CmlHiveAssistChatAttachmentPayload in
            CmlHiveAssistChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CmlHiveAssistChatSessionEntry {
        CmlHiveAssistChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: CmlHiveAssistChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CmlHiveAssistChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CmlHiveAssistAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CmlHiveAssistChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            CmlHiveAssistPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> CmlHiveAssistPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
