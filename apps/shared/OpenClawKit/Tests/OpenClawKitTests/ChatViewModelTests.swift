...
import Foundation
import Testing
@testable import CmlHiveAssistChatUI
...
private final class TestChatTransport: @unchecked Sendable, CmlHiveAssistChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CmlHiveAssistChatHistoryPayload]
    private let sessionsResponses: [CmlHiveAssistChatSessionsListResponse]

    private let stream: AsyncStream<CmlHiveAssistChatTransportEvent>
    private let continuation: AsyncStream<CmlHiveAssistChatTransportEvent>.Continuation

    init(
        historyResponses: [CmlHiveAssistChatHistoryPayload],
        sessionsResponses: [CmlHiveAssistChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<CmlHiveAssistChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CmlHiveAssistChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CmlHiveAssistChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [CmlHiveAssistChatAttachmentPayload]) async throws -> CmlHiveAssistChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return CmlHiveAssistChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CmlHiveAssistChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CmlHiveAssistChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CmlHiveAssistChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = CmlHiveAssistChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = CmlHiveAssistChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { CmlHiveAssistChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                CmlHiveAssistAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = CmlHiveAssistChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = CmlHiveAssistChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = CmlHiveAssistChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { CmlHiveAssistChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
