...
@MainActor
public struct CmlHiveAssistChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: CmlHiveAssistChatViewModel
    ...
    public init(
        viewModel: CmlHiveAssistChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                CmlHiveAssistChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [CmlHiveAssistChatMessage] {
        let base: [CmlHiveAssistChatMessage]
    ...
    private func mergeToolResults(in messages: [CmlHiveAssistChatMessage]) -> [CmlHiveAssistChatMessage] {
        ...
        for message in messages {
            ...
            let merged = CmlHiveAssistChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: CmlHiveAssistChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: CmlHiveAssistChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CmlHiveAssistChatMessage) -> String {
