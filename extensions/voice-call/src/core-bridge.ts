...
function resolveCmlHiveAssistRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.CML_HIVE_ASSIST_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["cml-hive-assist"]) {
  ...
  throw new Error("Unable to resolve core root. Set CML_HIVE_ASSIST_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveCmlHiveAssistRoot(), "dist", "extensionAPI.js");
