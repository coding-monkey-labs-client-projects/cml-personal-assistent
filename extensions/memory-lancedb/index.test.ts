...
const liveEnabled = HAS_OPENAI_KEY && process.env.CML_HIVE_ASSIST_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cml-hive-assist-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
