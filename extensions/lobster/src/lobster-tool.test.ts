...
async function writeFakeLobsterScript(
  scriptBody: string,
  prefix = "cml-hive-assist-lobster-plugin-",
) {
...
function fakeCtx(
  overrides: Partial<CmlHiveAssistPluginToolContext> = {},
): CmlHiveAssistPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "cml-hive-assist-lobster-plugin-noisy-",
    );
  ...
  it("rejects invalid JSON from lobster", async () => {
    const { dir } = await writeFakeLobsterScript(
      ...
      "cml-hive-assist-lobster-plugin-bad-",
    );
