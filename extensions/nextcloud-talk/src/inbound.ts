...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as CmlHiveAssistConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as CmlHiveAssistConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as CmlHiveAssistConfig);
