/**
 * Twitch channel plugin for CmlHiveAssist.
 *
 ...
 * for CmlHiveAssist. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: CmlHiveAssistConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: CmlHiveAssistConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: CmlHiveAssistConfig): boolean => {
