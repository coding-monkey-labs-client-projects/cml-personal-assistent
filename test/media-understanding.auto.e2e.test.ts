...
import type { CmlHiveAssistConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("cml-hive-assist-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("cml-hive-assist-bin-sherpa-");
      const modelDir = await makeTempDir("cml-hive-assist-sherpa-model-");
      ...
      const cfg: CmlHiveAssistConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("cml-hive-assist-bin-whispercpp-");
      const modelDir = await makeTempDir("cml-hive-assist-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("cml-hive-assist-bin-gemini-");
      ...
      const cfg: CmlHiveAssistConfig = { tools: { media: { image: {} } } };
