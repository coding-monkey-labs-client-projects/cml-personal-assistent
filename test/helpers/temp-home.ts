...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CML_HIVE_ASSIST_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.CML_HIVE_ASSIST_STATE_DIR = path.join(base, ".cml-hive-assist");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(
    path.join(os.tmpdir(), opts.prefix ?? "cml-hive-assist-test-home-"),
  );
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".cml-hive-assist", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
